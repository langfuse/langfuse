/*
  Theme Lab - dark-mode surface ladder + typography tuner for Langfuse
  (web/src/styles/globals.css). Paste into the DevTools console on any page of
  the running app (localhost:3000, dark mode).
  Ladder tokens are overridden inline on <html> via setProperty. Text-size
  tokens are Tailwind v4 "@theme inline" values compiled to LITERAL font-sizes
  in the .text-* utilities, so runtime custom-property overrides cannot reach
  them - the typography section instead injects a managed
  <style id="theme-lab-typography"> tag with !important rules.
  Notes for maintainers: this file is also the source for the bookmarklet, so
  it deliberately avoids literal percent and hash characters (PCT below,
  rgb() colors), avoids comment delimiters inside strings (CO/CC below), and
  uses block comments only.
*/
(function () {
  "use strict";

  var PANEL_ID = "theme-lab-panel";
  var LS_KEY = "themeLabOverrides";
  var TYPO_STYLE_ID = "theme-lab-typography";
  var PRECONNECT_ID = "theme-lab-fonts-preconnect";
  ...
  var PCT =
    String.fromCharCode(
      37,
    ); /* literal percent sign, kept out of source for bookmarklet safety */
  var CO = "/" + "*"; /* comment-open, split so the bookmarklet */
  var CC = "*" + "/"; /* generator's comment stripper cannot eat it */
  ...
  var TEXT_TIERS = [
    ...
    { key: "meta", label: "meta", prop: "--muted-foreground", vs: "--background" },
    { key: "faint", label: "faint", prop: "--foreground-tertiary", vs: "--background" },
    { key: "sidebar", label: "sidebar", prop: "--sidebar-foreground", vs: "--sidebar-background" },
    { key: "link", label: "link", prop: "--link", vs: "--background" },
    { key: "linkHover", label: "link-hover", prop: "--link-hover", vs: "--background" },
  ];
  /* The vs surfaces are ladder tiers; map prop -> ladder state key so contrast
     tracks live ladder edits. */
  var VS_LADDER_KEY = { "--background": "canvas", "--sidebar-background": "chrome" };
  ...
  var ACCENT_TIERS = [
    { key: "primaryAccent", label: "primary", prop: "--primary-accent", vsProp: "--background" },
    { key: "accentFill", label: "accent", prop: "--accent", vsProp: null },
    { key: "accentFg", label: "accent-fg", prop: "--accent-foreground", vsProp: "--accent" },
    { key: "ring", label: "ring", prop: "--ring", vsProp: "--background" },
    { key: "sbAccentFill", label: "sb-accent", prop: "--sidebar-accent", vsProp: null },
    { key: "sbAccentFg", label: "sb-acc-fg", prop: "--sidebar-accent-foreground", vsProp: "--sidebar-accent" },
  ];
  ...
  function parseTriplet(v) {
    ...
    if (parts.length >= 3) {
      var l = parseFloat(parts[parts.length - 1]);
      if (isFinite(l)) return { h: parts[0], s: parts[1], l: l };
    }
    return { h: "0", s: "0" + PCT, l: 0 };
  }

  /* Baseline = the stylesheet values, read with any inline override temporarily removed. */
  var baseline = {};
  function captureBaseline(prop) {
    var inline = root.style.getPropertyValue(prop);
    if (inline) root.style.removeProperty(prop);
    baseline[prop] = parseTriplet(getComputedStyle(root).getPropertyValue(prop));
    if (inline) root.style.setProperty(prop, inline);
  }
  ALL.forEach(function (tier) {
    tier.props.forEach(captureBaseline);
  });
  TEXT_TIERS.forEach(function (t) {
    captureBaseline(t.prop);
  });
  ACCENT_TIERS.forEach(function (t) {
    captureBaseline(t.prop);
  });

  function tripletFor(prop, l) {
    var b = baseline[prop];
    return b.h + " " + b.s + " " + fmtNum(l) + PCT;
  }
  ...
  var WEIGHTS = [400, 450, 500, 600, 700];
  var typo = { pairing: "default", sizes: {}, weights: {} };
  TYPE_TOKENS.forEach(function (t) {
  ...
  var textColorsStore = { dark: {}, light: {} };

  function baselineHsl(prop) {
    var b = baseline[prop];
  ...
  var accentColorsStore = { dark: {}, light: {} };

  /* Drawer UI prefs (width in px, collapsed-to-rail), persisted alongside
     the token overrides. Defaults: 320px, expanded. */
  var UI_DEFAULT_WIDTH = 320;
  var UI_MIN_WIDTH = 260;
  var UI_MAX_WIDTH = 560;
  var ui = { width: UI_DEFAULT_WIDTH, collapsed: false };
  ...
  function typoSizeChanged(t) {
    return Math.abs(typo.sizes[t.key] - t.def) > 0.001;
  }

  function typoWeightChanged(t) {
    return typo.weights[t.key] != null;
  }

  function typoRowChanged(t) {
    return typoSizeChanged(t) || typoWeightChanged(t);
  }

  function applyTextTier(t) {
    root.style.setProperty(t.prop, tripletFor(t.prop, textState[t.key]));
  }

  function isTextChanged(t) {
    return Math.abs(textState[t.key] - baseline[t.prop].l) > 0.001;
  }

  /* ---------- WCAG contrast between two HSL triplets ---------- */

  /* n mod m without the modulo operator (kept out of source: a literal
     percent char is unsafe inside a javascript: bookmarklet URL) */
  function mod(n, m) {
  ...
  function contrastRatio(t1, t2) {
    var l1 = relLum(hslToRgb(t1.h, t1.s, t1.l));
    var l2 = relLum(hslToRgb(t2.h, t2.s, t2.l));
    var hi = Math.max(l1, l2);
    var lo = Math.min(l1, l2);
    return (hi + 0.05) / (lo + 0.05);
  }

  function hslOfProp(prop, l) {
    var b = baseline[prop];
    return { h: parseFloat(b.h) || 0, s: parseFloat(b.s) || 0, l: l };
  }

  function textContrast(t) {
    var fg = hslOfProp(t.prop, textState[t.key]);
    var bg = hslOfProp(t.vs, state[VS_LADDER_KEY[t.vs]]);
    return contrastRatio(fg, bg);
  }

  function hslTripletStr(a) {
    return fmtNum(a.h) + " " + fmtNum(a.s) + PCT + " " + fmtNum(a.l) + PCT;
  }

  function accentTripletStr(t) {
  ...
  function isAccentChanged(t) {
    var b = baselineHsl(t.prop);
    var a = accentState[t.key];
    return (
  ...
  function computedHsl(prop) {
    var tr = parseTriplet(getComputedStyle(root).getPropertyValue(prop));
    return { h: parseFloat(tr.h) || 0, s: parseFloat(tr.s) || 0, l: tr.l };
  }

  /* hex round-trip for the native color picker (hash char built at runtime,
     kept out of source for bookmarklet safety) */
  ...
  function hslToHex(h, s, l) {
    ...
    var out = HASH;
    for (var i = 0; i < 3; i++) {
      var v = Math.round(Math.min(1, Math.max(0, rgb[i])) * 255);
  ...
  function ensureFontLoaded(family, id, url) {
    var has = false;
    try {
  ...
  function applyTypography() {
    ...
    PAIRINGS.forEach(function (p) {
      if (p.val === typo.pairing) pairing = p;
    });
    ...
    var tag = document.getElementById(TYPO_STYLE_ID);
    if (!css) {
      if (tag) tag.remove();
      return;
    }
  ...
  function save() {
    ...
    TYPE_TOKENS.forEach(function (t) {
      ...
      if (typoWeightChanged(t)) {
        ...
        out.typo.weights[t.key] = typo.weights[t.key];
        any = true;
      }
    });
  ...
  /* Re-apply persisted overrides (panel survives page navigations when re-injected). */
  (function () {
    var saved = null;
    try {
    ...
    if (saved && typeof saved === "object") {
      ...
      if (tp && typeof tp === "object") {
        ...
        var pv = tp.pairing === "system" ? "default" : tp.pairing;
        if (pv === "inter" || pv === "plex") typo.pairing = pv;
        if (tp.sizes && typeof tp.sizes === "object") {
          TYPE_TOKENS.forEach(function (t) {
            var v = tp.sizes[t.key];
            if (typeof v === "number" && isFinite(v)) {
              if (v > 8) v = v / 16; /* migrate old px persistence to rem */
              typo.sizes[t.key] = Math.min(2.5, Math.max(0.5, v));
            }
          });
        }
        if (tp.weights && typeof tp.weights === "object") {
          TYPE_TOKENS.forEach(function (t) {
            var v = tp.weights[t.key];
            if (WEIGHTS.indexOf(v) !== -1) typo.weights[t.key] = v;
          });
        }
        applyTypography();
      }
      var tc = saved.textColors;
      if (tc && typeof tc === "object") {
        ["dark", "light"].forEach(function (m) {
          if (tc[m] && typeof tc[m] === "object") {
            TEXT_TIERS.forEach(function (t) {
              var v = tc[m][t.key];
              if (typeof v === "number" && isFinite(v)) {
        ...
        TEXT_TIERS.forEach(function (t) {
          var v = textColorsStore[MODE][t.key];
          if (typeof v === "number") {
            textState[t.key] = v;
            applyTextTier(t);
          }
      ...
      var ac = saved.accentColors;
      if (ac && typeof ac === "object") {
        ["dark", "light"].forEach(function (m) {
          if (ac[m] && typeof ac[m] === "object") {
            ACCENT_TIERS.forEach(function (t) {
              var v = ac[m][t.key];
              if (
                v &&
        ...
        /* only the set matching the page's current mode is applied */
        ACCENT_TIERS.forEach(function (t) {
          var v = accentColorsStore[MODE][t.key];
          if (v) {
  ...
  collapseBtn.textContent = String.fromCharCode(187); /* chevrons pointing right */
  ...
  function makeSection(titleText, open) {
  ...
  function makeSwatch(titleText, getHex, onPick) {
  ...
  var ladderSection = makeSection("surface ladder", true);
  ...
  function updateGaps() {
    gapEls.forEach(function (el, i) {
      var below = state[TIERS[i].key];
      var above = state[TIERS[i + 1].key];
      var gap = above - below;
      var txt = (gap >= 0 ? "+" : "") + fmtNum(gap) + "pp";
      if (gap < 0) {
  ...
  function makeBtn(text, titleText, onClick) {
  ...
  PRESETS.forEach(function (p) {
    presetRow.appendChild(
      makeBtn(p.name, "Apply preset: " + JSON.stringify(p.vals), function () {
        applyPreset(p.vals);
      }),
    );
  });
  ...
  var typoSection = makeSection("typography", false);
  typoSection.style.marginTop = "6px";
  body.appendChild(typoSection);

  var pairingRow = document.createElement("div");
  ...
  PAIRINGS.forEach(function (opt) {
    ...
    r.value = opt.val;
    r.style.cssText =
      "margin:0;accent-color:rgb(150,140,255);cursor:pointer;";
  ...
  function refreshTypo(t) {
    ...
    el.weight.value =
      typo.weights[t.key] != null ? String(typo.weights[t.key]) : "";
    markTypo(t);
  }

  function makeTypeRow(t) {
    var row = document.createElement("div");
    row.style.cssText = "display:flex;align-items:center;gap:6px;margin:2px 0;";
    ...
    var pxHint = document.createElement("span");
    ...
    defOpt.value = "";
    defOpt.textContent = String.fromCharCode(8211); /* en dash */
    weight.appendChild(defOpt);
    WEIGHTS.forEach(function (wv) {
    ...
    weight.addEventListener("change", function () {
      typo.weights[t.key] = weight.value === "" ? null : parseInt(weight.value, 10);
      applyTypography();
      save();
      markTypo(t);
    });
  ...
  function sampleWeights() {
    ...
    TYPE_TOKENS.forEach(function (t) {
      var ui = typoInputs[t.key];
      if (!ui) return;
      var tally = {};
      var sampled = 0;
      try {
        var els = document.querySelectorAll(".text-" + t.key);
        for (var i = 0; i < els.length && sampled < 200; i++) {
          if (panel.contains(els[i])) continue;
          var fw = getComputedStyle(els[i]).fontWeight;
          if (!fw) continue;
      ...
      var baseTitle = "font-weight override for .text-" + t.key;
  ...
  typoSection.querySelector("summary").appendChild(rescanBtn);

  /* ---------- text colors section (collapsed by default) ---------- */

  var textSection = makeSection("text colors", false);
  textSection.style.marginTop = "6px";
  body.appendChild(textSection);

  function contrastColor(ratio) {
  ...
  function updateTextContrast() {
    TEXT_TIERS.forEach(function (t) {
      var el = textInputs[t.key];
      if (!el) return;
      var ratio = textContrast(t);
  ...
  var accentSection = makeSection("accent colors", false);
  accentSection.style.marginTop = "6px";
  body.appendChild(accentSection);

  function updateAccentContrast() {
    ACCENT_TIERS.forEach(function (t) {
      var el = accentInputs[t.key];
      if (!el || !t.vsProp) return;
      var ratio = contrastRatio(computedHsl(t.prop), computedHsl(t.vsProp));
      el.ratio.textContent = fmtNum(ratio) + ":1";
      el.ratio.style.color = contrastColor(ratio);
    });
  }
  ...
  function makeAccentRow(t) {
    ...
    function makeNum(min, max, step, titleText, width) {
    ...
    var hNum = makeNum(0, 360, 1, "hue (0-360)", 34);
    var sNum = makeNum(0, 100, 0.5, "saturation (0-100)", 34);
    var lNum = makeNum(0, 100, 0.1, "lightness (0-100)", 38);
    ...
    function commitPart(part, v, min, max) {
      if (!isFinite(v)) return;
    ...
    lNum.addEventListener("input", function () {
      var v = parseFloat(lNum.value);
      if (!isFinite(v)) return;
      slider.value = String(Math.min(100, Math.max(0, v)));
      commitPart("l", v, 0, 100);
    });
    [hNum, sNum, lNum].forEach(function (n) {
  ...
  function copyCss(btn) {
    ...
    TYPE_TOKENS.forEach(function (t) {
      ...
      if (typoWeightChanged(t)) {
        ...
        typoLines.push(
          CO + " text-" + t.key + ": weight " + typo.weights[t.key] + " " + CC,
        );
      }
    });
    if (typoLines.length || typo.pairing !== "default") {
      if (lines.length) lines.push("");
      lines.push(CO + " typography (Tailwind @theme inline text tokens) " + CC);
      typoLines.forEach(function (l) {
        lines.push(l);
      });
      if (typo.pairing !== "default") {
        PAIRINGS.forEach(function (p) {
          if (p.val === typo.pairing) {
            lines.push(
              CO + " pairing: " + p.label + " (bold " + p.bold + ") " + CC,
            );
          }
        });
      }
    ...
    if (textLines.length) {
      if (lines.length) lines.push("");
      lines.push(CO + " text colors (" + MODE + ") " + CC);
    ...
    if (accentLines.length) {
      if (lines.length) lines.push("");
      lines.push(CO + " accent colors (" + MODE + ") " + CC);
      accentLines.forEach(function (l) {
        lines.push(l);
      });
    }
    if (!lines.length) {
  ...
  /* The per-mode override sets (textColors.*, accentColors.*) are inline and
     therefore mode-blind once applied: when next-themes flips the dark class,
     the old mode's values would keep winning. Watch the class attribute and
     swap the mode-scoped sections over: drop all inline overrides, recapture
     the new mode's stylesheet baselines (H/S can differ per mode), re-apply
     the set stored for the new mode, and refresh rows. The surface ladder and
     typography are mode-agnostic by design (the ladder is dark-tuning
     oriented; its inline values persist across modes). */
  function handleModeFlip() {
    var newMode = root.classList.contains("dark") ? "dark" : "light";
  ...
  var modeObserver = new MutationObserver(handleModeFlip);
