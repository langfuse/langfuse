...
- Update this file when user feedback adds a durable repo-level instruction that
  ...
  `skills/` as well when that feedback changes a reusable workflow, checklist,
  or decision rule that those skills should teach. Treat feedback as durable
  when it changes the default workflow, review expectations, naming rules,
  verification rules, or handoff conventions for future tasks. Do not edit this
  file or shared skills for one-off task preferences.
...
- When you write a test for a bug or similar, write the test that fails first.
  Check that it fails. Only then fix the bug. Otherwise, the test is not good.
...
- Codex may create or refine shared skills under `skills/` when a repeated repo-specific workflow would help future agents. Keep shared skills tool-neutral and scoped to durable guidance.
