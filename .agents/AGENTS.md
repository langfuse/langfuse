...
- Opening a PR spins up a disposable, full-stack preview at
  `pr-<N>.preview.langfuse.com` — use the `langfuse-previews` skill to spin one
  up, debug it, or read a preview's web/worker error logs with `kubectl`.
