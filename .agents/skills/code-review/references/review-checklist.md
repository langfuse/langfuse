...
- Materialized views must never be dropped and recreated while their source table receives inserts — rows written between `DROP` and `CREATE` are silently and permanently missing from the target table. Redefine the SELECT with `ALTER TABLE <mv> MODIFY QUERY ...` instead, altering the target table(s) first when columns change (with `SETTINGS alter_sync = 2` on those `ALTER`s in `clustered/` files).
- `alter_sync` has no effect on `CREATE`/`DROP` statements (it only applies to `ALTER`/`OPTIMIZE`/`TRUNCATE`), and consecutive `ON CLUSTER` DDLs execute in order on each host via the distributed DDL queue — so `DROP VIEW` + `CREATE VIEW` pairs need no extra sync settings. `mutations_sync` is likewise irrelevant for view DDL and `MODIFY QUERY`: both are metadata-only and rewrite no data parts, so no mutation is created.
