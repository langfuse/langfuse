...
- Turning a feature into Linear subtickets that map onto an intended PR stack, and only when they explicitly ask to plan that stack: use `linear-planning`. Ordinary "create a subticket" / "file this" writes stay short under `linear-agent-writes`.
