...
- Identity: read `~/.config/langfuse/me.md`, or Cloud run owner / desktop `gh` (see `AGENTS.md`).
...
- Before any agent write to Linear — a comment, a description edit, a ticket: use `linear-agent-writes`, the write policy (propose updates; mark every write). Subtickets under an existing parent are free.
