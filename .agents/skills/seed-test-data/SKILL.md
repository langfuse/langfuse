...
| SDK-style experiment run in v4 events (experiment filters × hidden `sdk-experiment` env) | `pnpm run seed -- experiment-run --items 20` (enriched in-process shape; add `--unenriched-children` for the cross-process shape where child spans carry no experiment fields)                                                        |
