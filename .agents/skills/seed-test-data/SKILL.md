...
| Session messages carrying Langfuse media references (inline image, several refs in one message, link-only payload) | `pnpm run seed -- session-shapes --shape media` — uploads the image/audio fixtures to MinIO and links them to the observation, so the inline chip and the "Media" strip both resolve (LFE-14815)                                                                                                                                    |
