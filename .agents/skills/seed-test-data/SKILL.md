...
| A project the v4 migration wizard still has work for (sidebar "Action required" item, status row, side panel)                                     | `pnpm run seed -- v4-migration-pending` (active trace-level evaluation rules count as affected evals, which puts the project into "Action needed"; `--evals N` for more. Traffic-derived signals — SDK versions, deprecated APIs — come from `system.query_log` and worker Redis caches and are not seeded) |
