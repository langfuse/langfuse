...
| Diverse v4 session shapes (chat / coding-agent / OpenAI ChatML tools / mixed) for the session-detail view | `pnpm run seed -- session-shapes --shape all` (`tools` has available definitions, assistant calls, tool results, and matching TOOL observations; `agent` has I/O on AGENT/TOOL with no GENERATION; v4 on by default) |
