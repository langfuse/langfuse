...
ClickHouse, v4 dev tables, Redis, S3-compatible storage, web app) with the
exact fix command for every failure. Do not debug Docker/ClickHouse manually
before running this.
...
| Session messages carrying Langfuse media references (inline image, several refs in one message, bare refs in a content array, link-only payload) | `pnpm run seed -- session-shapes --shape media` — uploads the image/audio/pdf fixtures to Floci and links them to the observation, so the inline chip and the "Media" strip both resolve (LFE-14815, LFE-9577)                                                                                                                      |
