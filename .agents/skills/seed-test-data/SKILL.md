...
| A realistic agent flow over a timeline (graph view + scrubbable timeline)               | `pnpm run seed -- agent-timeline --turns 6 --v4` (LangGraph refine loop planner→retriever→generator→critic→loop, staggered in time; add `--langgraph-metadata false` for the pure timing fallback)                                    |
