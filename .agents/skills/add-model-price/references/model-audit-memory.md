...
The 2026-08-22 run re-fetched the full Anthropic pricing table, the full OpenAI
standard-pricing-table dump (plus Fast mode and Flex tables), and the Gemini AI Studio
pricing pages (`ai.google.dev/pricing` and `ai.google.dev/gemini-api/docs/models`) and
confirmed every price below unchanged from the 2026-08-21 snapshot, with two
exceptions: `gpt-5.6-sol` had a price cut (all 6 tiers updated), and a new model
`gpt-5.6-cyber` was found and added. See `provider-sources-and-price-keys.md` for the
full verification trail (a broad WebFetch also asserted a fabricated `gpt-5.5-cyber`
that did not survive independent verification and was not added). Rows below still
read "Re-confirmed ... [date of last dedicated verification]" where that earlier
wording remains accurate; treat any row without an explicit 2026-08-22 note as
re-confirmed today via one of these full-table fetches.
...
| OpenAI | gpt-5.6-sol | Input $4/MTok, Cached $0.40/MTok, Cache write $5.00/MTok, Output $20/MTok; Fast mode $8/$0.80/$10/$40, Fast+Large Context $16/$1.60/$20/$60; Flex $2/$0.20/$2.50/$10, Flex+Large Context $4/$0.40/$5/$15 | Yes | Large Context (>272K): $8/$0.80/$10/$30 | Yes | Updated | https://developers.openai.com/api/docs/pricing https://developers.openai.com/api/docs/models/gpt-5.6-sol | **Price cut found 2026-08-22.** Down from $5/$0.50/$6.25/$30. Confirmed via 3 independent fetches (aggregate table, dedicated model page, and a second targeted aggregate-table fetch) after a discrepancy was flagged; see provider-sources-and-price-keys.md. All 6 pricing-file tiers updated. |
| OpenAI | gpt-5.6-terra | Input $2/MTok, Cached $0.20/MTok, Cache write $2.50/MTok, Output $12/MTok | Yes | Large Context (>272K): $4/$0.40/$5.00/$18 | Yes | None | https://developers.openai.com/api/docs/pricing | Re-confirmed stable 2026-08-22. |
| OpenAI | gpt-5.6-luna | Input $0.20/MTok, Cached $0.02/MTok, Cache write $0.25/MTok, Output $1.20/MTok | Yes | Large Context (>272K): $0.40/$0.04/$0.50/$1.80 | Yes | None | https://developers.openai.com/api/docs/pricing | Re-confirmed stable 2026-08-22. |
| OpenAI | gpt-5.6-cyber | Input $12.50/MTok, Cached $1.25/MTok, Cache write $15.625/MTok, Output $75/MTok | Yes | Large Context (>272K): $25/$2.50/$31.25/$112.50 | Yes | Added | https://developers.openai.com/api/docs/models/gpt-5.6-cyber https://developers.openai.com/api/docs/models/all https://developers.openai.com/api/docs/pricing | **New model found 2026-08-22.** "Our most advanced cybersecurity model for authorized vulnerability research and security testing." 400k context window, 128k max output, supports reasoning tokens, no Fast/Flex pricing documented, Responses API only. Added to pricing file (Standard + Large Context tiers, mirroring the gpt-5.6-sol reasoning/no-caching-family key shape) and to `openAIModels` in types.ts (not first). A same-run WebFetch also claimed a `gpt-5.5-cyber` existed; independently checked on `api/docs/models/all` and found no trace of it — not added. |
...
| OpenAI | gpt-5.3-codex | Input $1.75/MTok, Cached $0.175/MTok, Output $14.00/MTok | Yes | No large-context tier (400k context window, single tier) | Yes | None | https://developers.openai.com/api/docs/pricing https://developers.openai.com/api/docs/models/gpt-5.3-codex | Re-confirmed unchanged 2026-08-22 via a dedicated fetch of the model's own page. |
...
| OpenAI | gpt-5-chat-latest | Input $1.25/MTok, Cached $0.125/MTok, Output $10/MTok | Yes | No provider tiering (128,000 token context window) | Yes | None | https://developers.openai.com/api/docs/models/gpt-5-chat-latest | **Re-verified 2026-08-22** via a dedicated fetch of the model's own page (still absent from the aggregate standard-pricing-table dump, consistent with every prior audit). Confirmed unchanged. |
...
## Unresolved findings (updated 2026-08-22)
