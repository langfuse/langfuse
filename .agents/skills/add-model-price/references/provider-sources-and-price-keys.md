...
- **Anthropic `/docs/en/about-claude/models/overview` footnote markers can glom onto Bedrock IDs in fetched markdown (found August 9 2026)** — A WebFetch of `https://platform.claude.com/docs/en/about-claude/models/overview` rendered table cells like "anthropic.claude-fable-53", "anthropic.claude-opus-4-86", and "anthropic.claude-opus-4-76" for the AWS Bedrock ID row. These are NOT real model-ID suffixes: the page uses numeric footnote reference markers (e.g. footnote 3: "Claude Fable 5, Claude Opus 5, and Claude Sonnet 5 are available on Bedrock through..."; footnote 6: "Claude Opus 4.8 and Claude Opus 4.7 are available on Bedrock through...") that sit immediately after the ID text in the source HTML, and the markdown conversion sometimes concatenates the footnote digit onto the preceding token with no separating space. The real Bedrock IDs are `anthropic.claude-fable-5`, `anthropic.claude-opus-4-8`, `anthropic.claude-opus-4-7`, etc. (confirmed against the pricing page and the existing, unchanged `matchPattern` entries in the pricing file, which already match the correct un-suffixed IDs). Before trusting any digit appended directly to a Bedrock/Google Cloud ID scraped from this specific overview page, cross-check it against the dedicated pricing page or an existing pricing-file `matchPattern` — do not add or "correct" a matchPattern based on a footnote-glommed ID from this page alone.
