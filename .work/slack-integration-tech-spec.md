...
4. **Token Management**: Secure storage of Slack OAuth tokens using existing Action config encryption patterns
...
- **Database**: Reuse existing Action/ActionConfig schema with new "SLACK" discriminated union type
...
export const SlackActionConfigSchema = z.object({
  ...
  botToken: z.string(), // encrypted like webhook secrets
  displayBotToken: z.string(), // masked like webhook displaySecretKey
...
#### 4.1.1 Extend Existing Action Types
```typescript
// Extend packages/shared/src/domain/automations.ts
export type SlackActionConfig = {
  type: "SLACK";
  botToken: string;
  displayBotToken: string; // Masked version for UI
  teamId: string;
  teamName: string;
  channelId: string;
  channelName: string;
...
export type ActionConfig = 
  | WebhookActionConfig
  | SlackActionConfig; // Add this line
```

#### 4.1.2 No Database Schema Changes
- **Reuse Existing**: Extend existing `Action` table with new config type
- **No New Tables**: All Slack configuration stored in existing `Action.config` JSONB field
- **Leverage Existing**: Use existing project association, triggers, and execution logging

### 4.2 Core Service Implementation

#### 4.2.1 Slack Service with Official Libraries
```typescript
...
export class SlackService {
  private static getOAuthProvider() {
    return new InstallProvider({
      ...
      installationStore: {
        // Simple in-memory store for OAuth state (could be Redis in production)
        storeInstallation: async (installation) => {
          // We don't persist installations here - handled by our own database
          return;
        },
...
export async function createSlackAction(params: {
  ...
  // Encrypt bot token using existing patterns
  const encryptedToken = encrypt(slackConfig.botToken);
  ...
  return prisma.action.create({
    data: {
      ...
      config: {
        ...
        displayBotToken: maskToken(slackConfig.botToken),
      }
    }
  });
}
...
export async function executeSlackAction(
  ...
  try {
    // Decrypt bot token
    const botToken = decrypt(action.config.botToken);
    
    // Format message using template
    const blocks = formatEventAsBlocks(eventData, action.config.messageTemplate);
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const successUrl = new URL('/project/' + result.projectId + '/automations', req.headers.origin);
    ...
    const errorUrl = new URL('/project/' + req.query.projectId + '/automations', req.headers.origin);
...
export const slackMiddleware = protectedProjectProcedure
  .use(async ({ ctx, next }) => {
    try {
      return await next();
    } catch (error) {
  ...
  const { mutate: exchangeOAuth } = api.slack.oauthExchange.useMutation({
    onSuccess: (result) => {
      onChange({
        ...
        displayBotToken: maskToken(result.botToken),
      });
...
export default async function handler(
  ...
  try {
    ...
    res.redirect(`/project/${projectId}/settings/automations?slack_connected=true`);
  } catch (error) {
...
function validateOAuthState(state: string): { projectId: string; userId: string } {
  try {
...
describe('SlackService', () => {
  describe('exchangeOAuthCode', () => {
    ...
    it('should handle OAuth errors', async () => {
      ...
      await expect(SlackService.exchangeOAuthCode({
        code: 'invalid-code',
        clientId: 'client-id',
        clientSecret: 'client-secret'
...
describe('Slack Error Handling', () => {
  ...
  it('should handle invalid tokens', async () => {
    ...
    await expect(SlackService.sendMessage({
...
- **No new database tables** - extends existing Action config schema
...
**Estimated implementation time: 1-2 weeks** vs 3-4 weeks for the complex approach, with much lower maintenance overhead and better integration with existing Langfuse patterns.