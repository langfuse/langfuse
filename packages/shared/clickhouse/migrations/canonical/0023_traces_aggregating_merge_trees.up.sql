...
CREATE TABLE traces_null {CLICKHOUSE_CLUSTER_CLAUSE}
(
    -- Identifiers
    `project_id`      String,
    `id`              String,
    ...
    -- Aggregations{CLICKHOUSE_CLUSTERED_ONLY: -- DO NOT USE}
...
-- Create the all AMT
CREATE TABLE traces_all_amt {CLICKHOUSE_CLUSTER_CLAUSE}
({CLICKHOUSE_CLUSTERED_ONLY:    }{CLICKHOUSE_UNCLUSTERED_ONLY:    }
    -- Identifiers
    `project_id`         String,
    `id`                 String,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS traces_all_amt_mv {CLICKHOUSE_CLUSTER_CLAUSE} TO traces_all_amt AS
SELECT
    -- Identifiers
...
CREATE TABLE traces_7d_amt {CLICKHOUSE_CLUSTER_CLAUSE}
(
    -- Identifiers
    `project_id`         String,
    `id`                 String,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS traces_7d_amt_mv {CLICKHOUSE_CLUSTER_CLAUSE} TO traces_7d_amt AS
SELECT
    -- Identifiers
    tn.project_id                                                                              as project_id,
    tn.id                                                                                      as id,
...
CREATE TABLE traces_30d_amt {CLICKHOUSE_CLUSTER_CLAUSE}
(
    -- Identifiers
    `project_id`         String,
    `id`                 String,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS traces_30d_amt_mv {CLICKHOUSE_CLUSTER_CLAUSE} TO traces_30d_amt AS
SELECT
    -- Identifiers
    tn.project_id                                                                              as project_id,
    tn.id                                                                                      as id,
    ...
    max(tn.updated_at)                                                                         as updated_at
FROM traces_null tn
GROUP BY project_id, id;{CLICKHOUSE_HISTORICAL_FINAL_NEWLINES:clustered}
