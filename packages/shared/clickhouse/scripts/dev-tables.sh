...
clickhouse client \
  ...
  -- Backfill experiment data from dataset_run_items
  -- This enriches events with experiment metadata for dataset runs
  -- We need to update existing events with experiment fields
  INSERT INTO events (project_id, trace_id, span_id, parent_span_id, start_time, end_time, name, type,
                      environment, version, release, tags, trace_name, user_id, session_id, public, bookmarked, level, status_message, completion_start_time, prompt_id,
                      prompt_name, prompt_version, model_id, provided_model_name, model_parameters,
                      provided_usage_details, usage_details, provided_cost_details, cost_details,
                      ...
                      experiment_item_root_span_id, experiment_item_expected_output,
                      experiment_item_metadata_names, experiment_item_metadata_values,
                      source, blob_storage_file_path, event_bytes,
                      created_at, updated_at, event_ts, is_deleted)
  -- Select from observations enriched with dataset run item metadata
  SELECT o.project_id,
         o.trace_id,
         o.id                                                                            AS span_id,
         CASE
         ...
         mapKeys(mapConcat(o.metadata, coalesce(t.metadata, map())))                     AS metadata_names,
         mapValues(mapConcat(o.metadata, coalesce(t.metadata, map())))                   AS metadata_raw_values,
         -- Experiment fields from dataset_run_items
         dri.dataset_run_id                                                              AS experiment_id,
         dri.dataset_run_name                                                            AS experiment_name,
         mapKeys(dri.dataset_run_metadata)                                               AS experiment_metadata_names,
         mapValues(dri.dataset_run_metadata)                                             AS experiment_metadata_values,
         ...
         coalesce(dri.observation_id, concat('t-', dri.trace_id))                        AS experiment_item_root_span_id,
         dri.dataset_item_expected_output                                                AS experiment_item_expected_output,
         mapKeys(dri.dataset_item_metadata)                                              AS experiment_item_metadata_names,
         mapValues(dri.dataset_item_metadata)                                            AS experiment_item_metadata_values,
         'ingestion-api-dual-write-experiments'                                          AS source,
         ''                                                                              AS blob_storage_file_path,
         byteSize(*)                                                                     AS event_bytes,
         o.created_at,
         o.updated_at,
         o.event_ts,
         o.is_deleted
  FROM dataset_run_items_rmt dri FINAL
  INNER JOIN observations o FINAL ON dri.project_id = o.project_id AND dri.trace_id = o.trace_id
  LEFT JOIN traces t FINAL ON o.project_id = t.project_id AND o.trace_id = t.id
  WHERE dri.is_deleted = 0 AND o.is_deleted = 0;

  -- Backfill experiment data from dataset_run_items for trace-level events
  INSERT INTO events (project_id, trace_id, span_id, parent_span_id, start_time, name, type,
                      environment, version, release, tags, trace_name, user_id, session_id, public, bookmarked, level,
                      model_parameters, provided_usage_details, usage_details, provided_cost_details, cost_details,
                      usage_pricing_tier_id, usage_pricing_tier_name,
  ...
  SELECT t.project_id,
         ...
         dri.dataset_item_id                                                             AS experiment_item_id,
         dri.dataset_item_version                                                        AS experiment_item_version,
         concat('t-', dri.trace_id)                                                      AS experiment_item_root_span_id,
         dri.dataset_item_expected_output                                                AS experiment_item_expected_output,
         mapKeys(dri.dataset_item_metadata)                                              AS experiment_item_metadata_names,
         mapValues(dri.dataset_item_metadata)                                            AS experiment_item_metadata_values,
         'ingestion-api-dual-write-experiments'                                          AS source,
         ...
         t.is_deleted
  FROM dataset_run_items_rmt dri FINAL
  INNER JOIN traces t FINAL ON dri.project_id = t.project_id AND dri.trace_id = t.id
  WHERE dri.is_deleted = 0 AND t.is_deleted = 0
    AND dri.observation_id IS NULL;  -- Only for traces without a specific observation

  -- Manually populate events_full from events
  -- The materialized view should do this automatically, but we do it explicitly to ensure data flows through
  ...
  SELECT
    ...
    coalesce(experiment_name, '') as experiment_name,
    coalesce(experiment_metadata_names, []) as experiment_metadata_names,
    coalesce(experiment_metadata_values, []) as experiment_metadata_values,
    coalesce(experiment_description, '') as experiment_description,
    coalesce(experiment_dataset_id, '') as experiment_dataset_id,
    coalesce(experiment_item_id, '') as experiment_item_id,
    experiment_item_version,
    coalesce(experiment_item_expected_output, '') as experiment_item_expected_output,
    coalesce(experiment_item_metadata_names, []) as experiment_item_metadata_names,
    coalesce(experiment_item_metadata_values, []) as experiment_item_metadata_values,
    coalesce(experiment_item_root_span_id, '') as experiment_item_root_span_id,
    source,
    coalesce(service_name, '') as service_name,
    coalesce(service_version, '') as service_version,
    coalesce(scope_name, '') as scope_name,
    coalesce(scope_version, '') as scope_version,
    coalesce(telemetry_sdk_language, '') as telemetry_sdk_language,
    coalesce(telemetry_sdk_name, '') as telemetry_sdk_name,
    coalesce(telemetry_sdk_version, '') as telemetry_sdk_version,
    blob_storage_file_path,
    event_bytes,
    created_at,
    updated_at,
  ...
  -- Manually populate events_core from events_full
  -- The materialized view should do this automatically, but we do it explicitly to ensure data flows through
