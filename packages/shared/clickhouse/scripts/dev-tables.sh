...
-- Create events_green table - lightweight version without input/output for fast queries
-- This table is populated via materialized view from the events table.
-- See LFE-5394 for ongoing discussion.
CREATE TABLE IF NOT EXISTS events_green
(
    project_id String,
    trace_id String,
    span_id String,
...
-- Create events_green_input_output table - stores input/output and metadata for full content retrieval
-- This table is populated via materialized view from the events table.
-- See LFE-5394 for ongoing discussion.
CREATE TABLE IF NOT EXISTS events_green_input_output
(
    project_id String,
    trace_id String,
    span_id String,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS events_green_mv TO events_green AS
SELECT
    project_id,
    trace_id,
    span_id,
...
-- Materialized view to populate events_green_input_output from events table
CREATE MATERIALIZED VIEW IF NOT EXISTS events_green_input_output_mv TO events_green_input_output AS
SELECT
    project_id,
    trace_id,
    span_id,
...
# BACKFILL QUERIES
# ============================================================================
# The following queries can be used to backfill the events_green and
# events_green_input_output tables from the events table per partition.
#
# Replace 'YYYYMM' with the target partition (e.g., '202501' for January 2025).
#
# These queries are NOT run automatically - run them manually as needed.
# ============================================================================

# --- Backfill events_green per partition ---
 INSERT INTO events_green
 SELECT
     project_id,
     trace_id,
     span_id,
     ...
     is_deleted
 FROM events e
 WHERE toYYYYMM(e.start_time) = '202601';

 --- Backfill events_green_input_output per partition ---
 INSERT INTO events_green_input_output
 SELECT
     project_id,
     trace_id,
     span_id,
