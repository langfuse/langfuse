...
/** The currency CHB reports accrued usage in; it rejects every other one. */
 ...
 *
 * A currency other than USD is treated as no reading: CHB asserts USD when it
...
export const chbPeriodUsageAmountUSD = (
  ...
  if (
    usage.currency != null &&
    usage.currency.toUpperCase() !== CHB_USAGE_CURRENCY
  ) {
    return null;
  }
  return usage.amount;
...
/**
 * The CHB REST configuration an app reads from its own environment. Optional
 * everywhere except the audience, which is defaulted in both env schemas.
 */
export type ChbApiClientEnv = {
  CLICKHOUSE_BILLING_BASE_URL?: string;
  CLICKHOUSE_BILLING_AUTH0_DOMAIN?: string;
  CLICKHOUSE_BILLING_AUTH0_CLIENT_ID?: string;
  CLICKHOUSE_BILLING_AUTH0_CLIENT_SECRET?: string;
  CLICKHOUSE_BILLING_AUTH0_AUDIENCE: string;
};

/**
 * Build a client from an app's env, or null when the CHB REST surface is not
 * fully configured. Callers treat null as "CHB unavailable" and fail closed — a
 * partially configured deployment must not reach CHB unauthenticated.
 *
 * The env is passed in rather than read here: web and worker each validate
 * their own environment, and this module is the transport both of them share.
 */
export const buildChbApiClient = (
  env: ChbApiClientEnv,
): ChbApiClient | null => {
  if (
