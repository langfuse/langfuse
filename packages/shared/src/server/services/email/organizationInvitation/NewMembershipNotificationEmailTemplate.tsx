...
export const NewMembershipNotificationTemplate = ({
  ...
  const previewText = `A new member has ${userExists ? "joined" : "been added to"} ${orgName} on Langfuse`;
        ...
        <Body className="mx-auto my-auto bg-background font-sans">
          <Container className="mx-auto my-10 w-[465px] rounded border border-solid border-[#eaeaea] p-5">
            ...
            <Heading className="mx-0 my-[30px] p-0 text-center text-2xl font-normal text-black">
              New member {userExists ? "joined" : "added to"}{" "}
            ...
            <Text className="text-sm leading-6 text-black">
              {userExists ? (
                <>
                  A new member with the email{" "}
                  <strong>{invitedByUserEmail}</strong> has
                  <strong> joined </strong> the <strong>{orgName}</strong>{" "}
                  organization, invited by <strong>{invitedByUsername}</strong>.
                </>
              ) : (
                <>
                  A new member with the email{" "}
                  <strong>{invitedByUserEmail}</strong> has been
                  <strong> added </strong> to the <strong>{orgName}</strong>{" "}
                  organization by <strong>{invitedByUsername}</strong>.
                </>
              )}
              {langfuseCloudRegion && (
                <span>
