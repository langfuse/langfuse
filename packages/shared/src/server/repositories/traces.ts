    ...
    newExecution: (input) => {
      const query = `
        SELECT
          ...
          finalizeAggregation(bookmarked) as bookmarked,
          tags,
          ${renderingProps.truncated ? `leftUTF8(finalizeAggregation(input), ${env.LANGFUSE_SERVER_SIDE_IO_CHAR_LIMIT})` : "finalizeAggregation(input)"} as input,
          ${renderingProps.truncated ? `leftUTF8(finalizeAggregation(output), ${env.LANGFUSE_SERVER_SIDE_IO_CHAR_LIMIT})` : "finalizeAggregation(output)"} as output,
          session_id as session_id,
          0 as is_deleted,
          start_time as timestamp,
          created_at,
