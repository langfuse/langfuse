...
export async function validateWebhookURL(urlString: string): Promise<void> {
  ...
  if (isIPAddress(hostname)) {
    if (isIPBlocked(hostname, env.LANGFUSE_WEBHOOK_WHITELISTED_IPS, env.LANGFUSE_WEBHOOK_WHITELISTED_IP_SEGMENTS)) {
  ...
  for (const ip of ips) {
    if (isIPBlocked(ip, env.LANGFUSE_WEBHOOK_WHITELISTED_IPS,env.LANGFUSE_WEBHOOK_WHITELISTED_IP_SEGMENTS)) {
