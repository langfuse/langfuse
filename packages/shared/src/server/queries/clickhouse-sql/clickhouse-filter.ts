...
export class StringObjectFilter implements Filter {
  ...
  apply(): ClickhouseFilter {
    ...
    if (isFtsEventsTable(this.clickhouseTable)) {
      ...
      // Presence operators (`is set` / `is not set`) match on the key alone,
      // so they stay on the canonical branch — the JSON value predicate does
      // not apply to them.
      const legacyPath =
        this.operator !== "is set" &&
        this.operator !== "is not set" &&
        isFtsMetadataField(this.field)
