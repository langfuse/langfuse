...
export const eventsTracesAggregation = (
  ...
  return `
	  SELECT
	      trace_id AS id,
	      project_id,
	      argMax(name, start_time) AS name,
	      min(start_time) as timestamp,
	      argMax(environment, start_time) AS environment,
	      argMax(version, start_time) AS version,
	      argMax(session_id, start_time) AS session_id,
	      argMax(user_id, start_time) AS user_id,
	      argMax(input, start_time) AS input,
	      argMax(output, start_time) AS output,
	      argMax(metadata, start_time) AS metadata,
