/**
 * Event Query Builder for OceanBase
 *
 * This is the OceanBase-adapted version of event-query-builder.
 * Key differences from ClickHouse version:
 * - Uses TIMESTAMPDIFF instead of date_diff
 * - Uses IF/CASE instead of if/multiIf
 * - Uses SUBSTRING instead of leftUTF8
 * - Uses JSON_OBJECTAGG instead of mapFromArrays
 * - Uses GROUP_CONCAT with SUBSTRING_INDEX instead of argMaxIf
 * - Uses SUM(CASE...) instead of countIf
 * - Uses GROUP_CONCAT(DISTINCT...) instead of groupUniqArrayIf
 * - Uses GREATEST (uppercase) instead of greatest
 */
...
const EVENTS_FIELDS = {
  ...
  totalCost:
    "CAST(JSON_UNQUOTE(JSON_EXTRACT(e.cost_details, '$.total')) AS DECIMAL(18,12)) as total_cost",
  ...
  input: "e.input",
  output: "e.output",
  metadata: "e.metadata as metadata",

  // Calculated fields
  latency:
    "IF(e.end_time IS NULL, NULL, TIMESTAMPDIFF(MICROSECOND, e.start_time, e.end_time) / 1000) as latency",
  timeToFirstToken:
    "IF(e.completion_start_time IS NULL, NULL, TIMESTAMPDIFF(MICROSECOND, e.start_time, e.completion_start_time) / 1000) as time_to_first_token",
} as const;
 ...
 * OceanBase version: Uses standard MySQL aggregation functions
 * Note: argMaxIf is replaced with SUBSTRING_INDEX(GROUP_CONCAT(...), ',', 1) pattern
 */
const EVENTS_AGGREGATION_FIELDS = {
  ...
  // Aggregated fields - OceanBase adaptations
  // Pattern: Use MAX/MIN with CASE or subquery to simulate argMaxIf
  name: "SUBSTRING_INDEX(GROUP_CONCAT(CASE WHEN trace_name <> '' THEN trace_name END ORDER BY event_ts DESC), ',', 1) AS name",
  timestamp: "min(start_time) as timestamp",
  environment:
    "SUBSTRING_INDEX(GROUP_CONCAT(CASE WHEN environment <> '' THEN environment END ORDER BY event_ts DESC), ',', 1) AS environment",
  version:
    "SUBSTRING_INDEX(GROUP_CONCAT(CASE WHEN version <> '' THEN version END ORDER BY event_ts DESC), ',', 1) AS version",
  session_id:
    "SUBSTRING_INDEX(GROUP_CONCAT(CASE WHEN session_id <> '' THEN session_id END ORDER BY event_ts DESC), ',', 1) AS session_id",
  user_id:
    "SUBSTRING_INDEX(GROUP_CONCAT(CASE WHEN user_id <> '' THEN user_id END ORDER BY event_ts DESC), ',', 1) AS user_id",
  input:
    "SUBSTRING_INDEX(GROUP_CONCAT(CASE WHEN parent_span_id = '' THEN input END ORDER BY event_ts DESC), ',', 1) AS input",
  output:
    "SUBSTRING_INDEX(GROUP_CONCAT(CASE WHEN parent_span_id = '' THEN output END ORDER BY event_ts DESC), ',', 1) AS output",
  input_truncated:
    "SUBSTRING_INDEX(GROUP_CONCAT(CASE WHEN parent_span_id = '' THEN LEFT(input, 1024) END ORDER BY event_ts DESC), ',', 1) AS input_truncated",
  output_truncated:
    "SUBSTRING_INDEX(GROUP_CONCAT(CASE WHEN parent_span_id = '' THEN LEFT(output, 1024) END ORDER BY event_ts DESC), ',', 1) AS output_truncated",
  metadata:
    "SUBSTRING_INDEX(GROUP_CONCAT(CASE WHEN parent_span_id = '' THEN CAST(metadata AS CHAR) END ORDER BY event_ts DESC), ',', 1) AS metadata",
  created_at: "MIN(created_at) AS created_at",
  updated_at: "MAX(updated_at) AS updated_at",
  total_cost:
    "SUM(CAST(JSON_UNQUOTE(JSON_EXTRACT(cost_details, '$.total')) AS DECIMAL(18,12))) AS total_cost",
  latency_milliseconds:
    "TIMESTAMPDIFF(MICROSECOND, min(start_time), GREATEST(max(start_time), max(end_time))) / 1000 AS latency_milliseconds",
  observation_ids:
    "GROUP_CONCAT(DISTINCT CASE WHEN span_id <> '' THEN span_id END) AS observation_ids",

  bookmarked:
    "SUBSTRING_INDEX(GROUP_CONCAT(CASE WHEN parent_span_id = '' THEN bookmarked END ORDER BY event_ts DESC), ',', 1) AS bookmarked",
  public: "MAX(`public`) AS `public`",

  // Observation-level aggregations for filtering support
  // OceanBase: sumMap not available, use JSON aggregation or separate columns
  usage_details: "JSON_OBJECTAGG('usage', usage_details) as usage_details",
  cost_details: "JSON_OBJECTAGG('cost', cost_details) as cost_details",
  aggregated_level:
  ...
  tags: "SUBSTRING_INDEX(GROUP_CONCAT(CASE WHEN JSON_LENGTH(tags) > 0 THEN tags END ORDER BY event_ts DESC), ',', 1) AS tags",
  release:
    "SUBSTRING_INDEX(GROUP_CONCAT(CASE WHEN `release` <> '' THEN `release` END ORDER BY event_ts DESC), ',', 1) AS `release`",
} as const;
...
abstract class AbstractQueryBuilder {
  ...
  protected positionalParams: unknown[] = [];

  /**
   * Add raw WHERE condition with positional parameters.
   * Use ? placeholders in the condition string.
   *
   * Example:
   *   .whereRaw("span_id = ?", [id])
   */
  whereRaw(condition: string, values?: unknown[]): this {
    ...
    if (values) {
      this.positionalParams.push(...values);
    }
    return this;
  }
   ...
   * Accepts named params from the filter system and converts them
   * to positional parameters automatically.
   */
  where(condition: { query: string; params?: Record<string, any> }): this {
    if (condition.query.trim()) {
      ...
      const converted = convertFilterParamsToPositional(
        trimmedQuery,
        (condition.params ?? {}) as Record<string, unknown>,
      );
      this.whereClauses.push(`(${converted.query})`);
      this.positionalParams.push(...converted.params);
    }
    return this;
  }
  ...
  limit(limit?: number, offset?: number): this {
    if (limit !== undefined && offset !== undefined && offset > 0) {
      ...
      this.positionalParams.push(limit, offset);
    } else if (limit !== undefined) {
      this.limitClause = "LIMIT ?";
      this.positionalParams.push(limit);
    } else {
      this.limitClause = "";
    }
    return this;
  ...
  buildWithParams(): { query: string; params: unknown[] } {
    return {
      query: this.buildQuery(),
      params: this.positionalParams,
    };
  }
...
abstract class AbstractCTEQueryBuilder extends AbstractQueryBuilder {
   ...
   * Accepts positional params (unknown[]).
   */
  withCTE(
    name: string,
    queryWithParams: { query: string; params: unknown[] },
  ): this {
    this.ctes.push(`${name} AS (${queryWithParams.query})`);
    this.positionalParams.push(...queryWithParams.params);
    return this;
  }
...
> extends AbstractCTEQueryBuilder {
  ...
  protected buildQuery(): string {
    ...
    if (this.projectId !== NoProjectId) {
      allWhereClauses.unshift("e.project_id = ?");
      this.positionalParams.unshift(this.projectId);
    }
...
> {
  ...
  protected buildSelectClause(): string {
    ...
    if (this.ioFields) {
      if (this.ioFields.truncated && this.ioFields.charLimit !== undefined) {
        ...
        fieldExpressions.push(
          `SUBSTRING(input_truncated, 1, ${this.ioFields.charLimit}) as input, SUBSTRING(output_truncated, 1, ${this.ioFields.charLimit}) as output`,
        );
      } else {
    ...
    if (
      ...
      fieldExpressions.push("e.metadata as metadata");
    }
...
> {
  ...
  withStartTimeFrom(startTimeFrom?: string | null): this {
    return this.when(Boolean(startTimeFrom), (b) =>
      b.whereRaw(
        `start_time >= DATE_SUB(?, ${OBSERVATIONS_TO_TRACE_INTERVAL})`,
        [startTimeFrom],
      ),
    );
  }
...
> extends AbstractQueryBuilder {
  ...
  withCTE<Name extends string, Schema extends CTESchema>(
    ...
    this.positionalParams.push(...cteWithSchema.params);
...
export class EventsAggQueryBuilder extends AbstractCTEQueryBuilder {
  ...
  protected buildQuery(): string {
    ...
    const allWhereClauses = ["e.project_id = ?", ...this.whereClauses];
    this.positionalParams.unshift(this.projectId);
    parts.push(`WHERE ${allWhereClauses.join("\n  AND ")}`);
