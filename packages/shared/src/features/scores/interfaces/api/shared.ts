...
/**
 * Foundation schema for all score types, needs to be extended with entity score may be associated with. Note there are two API versions, where v1 allows only trace and observation scores, while v2 additionally allows session and run scores
...
// POST /scores
// Please note that the POST /scores endpoint supports all score types (trace, session, run) across v1 and v2.
/**
...
// Please note that the DELETE /scores/{scoreId} endpoint supports all score types (trace, session, run) across v1 and v2.
