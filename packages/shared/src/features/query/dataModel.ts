...
export const eventsObservationsView: ViewDeclarationType = {
  ...
  dimensions: {
    ...
    // Filter-only dimensions (LFE-10751): observations-page filters exposed in
    // the widget/monitor form. uiHidden keeps them out of the breakdown picker.
    latency: {
      sql: "date_diff('millisecond', events_observations.start_time, events_observations.end_time) / 1000.0",
      alias: "latency",
      type: "number",
      description: "Latency of the observation in seconds.",
      uiHidden: true,
    },
    isRootObservation: {
      sql: "(events_observations.parent_span_id = '' OR events_observations.is_app_root = true)",
      alias: "isRootObservation",
      type: "boolean",
      description: "True when the observation is the root span of its trace.",
      uiHidden: true,
    },
    modelId: {
      sql: "events_observations.model_id",
      alias: "modelId",
      type: "string",
      description: "Internal model identifier resolved for the observation.",
      highCardinality: true,
      uiHidden: true,
    },
    statusMessage: {
      sql: "events_observations.status_message",
      alias: "statusMessage",
      type: "string",
      description: "Status message of the observation.",
      uiHidden: true,
    },
    timeToFirstToken: {
      sql: "date_diff('millisecond', events_observations.start_time, events_observations.completion_start_time) / 1000.0",
      alias: "timeToFirstToken",
      type: "number",
      description: "Time to first token of the observation in seconds.",
      uiHidden: true,
    },
    inputTokens: {
      sql: "arraySum(mapValues(mapFilter(x -> positionCaseInsensitive(x.1, 'input') > 0, usage_details)))",
      alias: "inputTokens",
      type: "number",
      description: "Sum of input tokens consumed by the observation.",
      uiHidden: true,
    ...
    outputTokens: {
      sql: "arraySum(mapValues(mapFilter(x -> positionCaseInsensitive(x.1, 'output') > 0, usage_details)))",
      alias: "outputTokens",
      type: "number",
      description: "Sum of output tokens produced by the observation.",
      uiHidden: true,
    ...
    totalTokens: {
      sql: "if(mapExists((k, v) -> (k = 'total'), usage_details), usage_details['total'], NULL)",
      alias: "totalTokens",
      type: "number",
      description: "Total tokens consumed by the observation.",
      uiHidden: true,
    },
    inputCost: {
      sql: "arraySum(mapValues(mapFilter(x -> positionCaseInsensitive(x.1, 'input') > 0, cost_details)))",
      alias: "inputCost",
      type: "number",
      description: "Sum of input cost incurred by the observation.",
      uiHidden: true,
    ...
    outputCost: {
      sql: "arraySum(mapValues(mapFilter(x -> positionCaseInsensitive(x.1, 'output') > 0, cost_details)))",
      alias: "outputCost",
      type: "number",
      description: "Sum of output cost incurred by the observation.",
      uiHidden: true,
    ...
    totalCost: {
      sql: "if(mapExists((k, v) -> (k = 'total'), cost_details), cost_details['total'], NULL)",
      alias: "totalCost",
      type: "number",
      description: "Total cost incurred by the observation.",
      uiHidden: true,
    ...
    toolDefinitions: {
      sql: "length(mapKeys(events_observations.tool_definitions))",
      alias: "toolDefinitions",
      type: "number",
      description: "Number of available tools defined for the observation.",
      uiHidden: true,
    },
    toolCalls: {
      sql: "length(events_observations.tool_calls)",
      alias: "toolCalls",
      type: "number",
      description: "Number of tool calls made by the observation.",
      uiHidden: true,
    },
  },
  measures: {
