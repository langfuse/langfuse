// The system table-view preset catalog. Lives in the domain layer (not the
// server TableViewService) because the definitions are plain data with
// client-safe deps — client code (e.g. the events-table preset tests) reads
// the catalog without pulling in the server barrel. The server service
// re-exports it, so server callers are unchanged.
import {
  SystemTableViewPresetCategory,
  type TableViewPresetState,
  TableViewPresetTableName,
} from "./table-view-presets";
