...
async function main() {
    ...
    if (environment === "examples" || environment === "load") {
        ...
        if (
            !(await prisma.apiKey.findUnique({ where: { id: secondKey.id } }))
        ) {
            await prisma.apiKey.create({
                data: {
                    note: secondKey.note,
        ...
        for (let datasetNumber = 0; datasetNumber < 2; datasetNumber++) {
            const dataset = await prisma.dataset.create({
                data: {
                    ...
                    metadata:
                        datasetNumber === 0 ? { key: "value" } : undefined,
                },
            });
                    ...
                    data: {
                        ...
                        expectedOutput:
                            Math.random() > 0.3
                                ...
                                : undefined,
                        metadata:
                            Math.random() > 0.5 ? { key: "value" } : undefined,
                    },
                });
                ...
                for (const datasetItemId of datasetItemIds) {
                    ...
                    const observation =
                        relevantObservations[
                            Math.floor(
                                Math.random() * relevantObservations.length
                            )
                        ];
...
async function uploadObjects(
    ...
    for (let i = 0; i < promises.length; i++) {
        if (
            i + 1 >= promises.length ||
            i % Math.ceil(promises.length / 10) === 0
        )
            logger.info(
...
function createObjects(
                ...
                ...(existingSpanIds.length === 0 || Math.random() > 0.5
                    ...
                    : {
                          parentObservationId:
                              existingSpanIds[
                                  Math.floor(
                                      Math.random() * existingSpanIds.length
                                  )
                              ],
                      }),
            };
            ...
            for (let k = 0; k < Math.floor(Math.random() * 2) + 1; k++) {
                ...
                const promptId =
                    promptIds.get(projectId)![
                        Math.floor(
                            Math.random() *
                                Math.floor(promptIds.get(projectId)!.length / 2)
                        )
                    ];
    ...
    const uniqueSessions: Prisma.TraceSessionCreateManyInput[] = Array.from(
        ...
        (session) => JSON.parse(session) as Prisma.TraceSessionCreateManyInput
    );
