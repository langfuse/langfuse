...
| `agent-timeline`   | one trace: a LangGraph-style refine loop (planner -> retriever -> generator -> critic) unrolled over N turns, staggered across a real timeline with `langgraph_node`/`langgraph_step` metadata; exercises real-flow-with-loop graph rendering and a scrubbable timeline                                                                                                            | `--turns`, `--turn-gap-ms`, `--timing-only`, `--v4`                                                                   |
| `agent-graph`      | one trace that is large as a GRAPH rather than a tree: many distinct `langgraph_node` names with parallel branches per super-step, so a few hundred observations yield thousands of distinct node-pair connections (defaults to roughly 1,350)                                                                                                                                     | `--nodes`, `--steps`, `--parallel`, `--v4`                                                                            |
| `deep-chain`       | one trace whose observations form a single deep parent chain of sequential generations (child starts after parent ends; depth = observation count) - the mis-parented-instrumentation shape that collapses tree and timeline layouts at extreme depth                                                                                                                              | `--observations` (default 1401), `--v4`                                                                               |
...
| `outlier-traffic`  | diurnal base traffic over the past N days with deterministic cost/latency/token outliers and hour-long latency incidents, for the outlier chart strip above the trace table; root AGENT + GENERATION (carries cost) + TOOL per trace                                                                                                                                               | `--days`, `--traces-per-day`, `--incidents`, `--v4` (default true)                                                    |
| `scored-traces`    | standalone traces with mixed current/legacy Python and JavaScript SDK attribution, carrying numeric and categorical scores whose names contain spaces, plus dual-level names present at BOTH observation and trace level on one trace - the level-agnostic scores filter and ScoreTag edge cases                                                                                   | `--traces`, `--v4`                                                                                                    |
| `session-shapes`   | diverse v4 session shapes for session detail: a clean multi-turn CHAT session, a coding/AGENT session whose I/O lives on AGENT/TOOL observations with NO GENERATION, a MIXED session, and a MEDIA session carrying `@@@langfuseMedia:...@@@` references; creates the Postgres `trace_sessions` rows, and the media shape uploads its assets to MinIO                               | `--shape chat\|agent\|mixed\|media\|all`, `--turns`, `--v4` (default true)                                            |
...
| `custom-models`    | project-level model definitions (one tiered, with a condition-gated second tier and a usage type priced at 0; one single-tier) plus a trace whose generations link to them, and one generation matching no definition so its badge opens the create dialog                                                                                                                         | `--v4`                                                                                                                |
| `support-agent`    | one demo-grade, handcrafted trace: a support copilot resolving a duplicate-charge refund - input guardrail -> intent classification -> parallel context fan-out -> 3-turn ReAct loop -> drafted reply -> output guardrail -> send; real-looking payloads and per-model cost, built for videos and screenshots                                                                      | `--v4`                                                                                                                |
| `timeline-shapes`  | a dozen SMALL traces (4-25 observations each), one per timeline morphology: rag answer, streamed chat, parallel fan-out, retry backoff, a long wait on a human, one slow tool dwarfing the rest, an error cascade with failover, in-flight spans with no end time, zero-duration checkpoints, a deep ladder, flat siblings, and an agent loop; hand-timed with plausible latencies | `--shape`, `--v4`                                                                                                     |
