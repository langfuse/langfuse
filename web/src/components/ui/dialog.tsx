...
const dialogContentVariants = cva(
  "fixed left-[50%] top-[50%] overflow-hidden flex w-full translate-x-[-50%] translate-y-[-50%] flex-col bg-elevated shadow-lg sm:rounded-lg",
  {
    variants: {
      size: {
      ...
      <DialogPortal>
        ...
        >
          ...
          <div className="[&:has(.dialog-header)]:hidden [&:not(:has(.dialog-header))]:absolute [&:not(:has(.dialog-header))]:top-3 [&:not(:has(.dialog-header))]:right-3 [&:not(:has(.dialog-header))]:z-20">
            <DialogPrimitive.Close className="ring-offset-canvas focus:ring-focus data-[state=open]:bg-hover data-[state=open]:text-tertiary rounded-sm opacity-70 transition-opacity hover:opacity-100 focus:ring-2 focus:ring-offset-2 focus:outline-hidden disabled:pointer-events-none">
              <X className="h-4 w-4" />
...
const dialogHeaderVariants = cva(
  "bg-elevated sticky top-0 z-30 flex shrink-0 flex-col space-y-1.5 rounded-t-lg p-4",
  {
    variants: {
      variant: {
        default: "border-b",
...
const DialogHeader = ({
  ...
  >
    <div className="flex w-full items-center justify-between gap-4 text-center sm:text-left">
      ...
      <DialogPrimitive.Close
        className="ring-offset-canvas focus:ring-focus data-[state=open]:bg-hover data-[state=open]:text-tertiary z-20 mt-1 ml-4 rounded-sm opacity-70 transition-opacity hover:opacity-100 focus:ring-2 focus:ring-offset-2 focus:outline-hidden disabled:pointer-events-none"
        tabIndex={-1}
      >
        <X className="h-4 w-4" />
        <span className="sr-only">Close</span>
...
const dialogFooterVariants = cva(
  "bg-elevated sticky bottom-0 z-10 flex shrink-0 flex-col-reverse rounded-b-lg p-6 px-6 sm:flex-row sm:justify-end sm:space-x-2",
  {
    variants: {
      variant: {
        default: "border-t",
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Description
    ref={ref}
    className={cn("text-tertiary mt-1 text-sm", className)}
    {...props}
  />
));
