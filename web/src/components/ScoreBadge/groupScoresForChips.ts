...
/** Code-point order, the order the chips always had. Not `localeCompare`:
    that folds case and punctuation, so the chips would reorder. */
...
export function groupScoresForChips<T extends { name: string }>(
  ...
  return groups.sort((a, b) => compareText(a.label, b.label));
}
