 ...
 * `EventsOutlierStrip` so the events, scores and experiments
 ...
 * the band's whole content box. A strip whose ready content is taller passes
 * its own, so the band does not resize the moment its data arrives.
 */
const DEFAULT_CONTENT_HEIGHT_CLASS = "h-[76px]";
...
export const MetricStripBand = forwardRef<
  HTMLDivElement,
  {
    ...
    emptyMessage?: string;
    /**
     * Height of the loading / empty box, for a strip whose ready content is
     * taller than the default. Must equal that content's height, or the band —
     * and the table under it — jumps when the data arrives.
     */
  ...
  (
    {
      ...
      contentHeightClass = DEFAULT_CONTENT_HEIGHT_CLASS,
      stale = false,
      header,
      children,
    },
    ...
    // Ruled top and bottom: the strip reads as its own band
