...
export default async function handler(
  ...
  try {
    ...
    const { client: s3Client, bucketName, prefix } = getS3EventStorageConfig();

    // Generate S3 prefixes for the time range (hour-level)
    const prefixes = generateOtelS3Prefixes(prefix, projectId, start, end);
    ...
    for (let i = 0; i < allFiles.length; i += BATCH_SIZE) {
      ...
      const jobs: Jobtyp = batch.map((fileKey) => ({
