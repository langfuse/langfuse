...
export const [listScoresTool, handleListScores] = defineTool({
  ...
  analyticsProperties: (input) => ({
    ...
    hasTraceField: input.fields.includes("trace"),
    userId: input.userId,
    dataType: input.dataType,
    configId: input.configId,
    queueId: input.queueId,
    traceTagCount: input.traceTags?.length,
    environmentCount: input.environment?.length,
    ...
    scoreIdCount: input.scoreIds?.length,
    sessionId: input.sessionId,
    traceId: input.traceId,
    datasetRunId: input.datasetRunId,
    observationIdCount: input.observationId?.length,
