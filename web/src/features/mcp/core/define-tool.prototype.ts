 ...
 * LFE-15559). defineTool with a required per-tool `action`, asserted before
 * argument validation. Per-tool auth is net-new — legacy ran no per-tool gate —
 * so in shadow the assert only observes (diffs `legacy = absent`); enforce gates.
 * Run: `pnpm --filter web run test:in-source define-tool.prototype`.
 */
...
import { diffResults } from "@/src/features/auth/policy/shadow.prototype";
import {
  defineTool,
...
/** assertToolAccess fails closed on a missing context; then in shadow it only observes (legacy ran no per-tool gate, so it diffs against `absent`), and in enforce it throws an ungranted action. */
function assertToolAccess(action: ProjectAction, context: ServerContext): void {
  ...
  if (authzMigrationMode === "shadow") {
    ...
    diffResults(decision, { absent: true }, { seam: "mcp_tool", action });
    return;
  }
