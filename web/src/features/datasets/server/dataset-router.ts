...
export const datasetRouter = createTRPCRouter({
    ...
    .query(async ({ input, ctx }) => {
      ...
      >`SELECT 
        ...
        WHERE 
          ...
          AND dri.dataset_run_id = ${input.datasetRunId}
        ORDER BY 
          di.created_at DESC,
          di.id DESC
        LIMIT ${input.limit}
  ...
  runItemsByItemId: protectedProjectProcedure
    ...
    .query(async ({ input, ctx }) => {
      const runItems = await ctx.prisma.$queryRaw<Array<RunItemTableRow>>`SELECT
          di.id AS "datasetItemId",
          di.created_at AS "datasetItemCreatedAt",
          dri.id,
          dri.trace_id AS "traceId",
        ...
        WHERE
          dri.project_id = ${input.projectId}
          AND dri.dataset_item_id = ${input.datasetItemId}
        ORDER BY
          di.created_at DESC,
          di.id DESC
        LIMIT ${input.limit}
