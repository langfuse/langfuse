...
export default function ObservationsEventsTable({
  ...
  // tables. The sidebar's FilterState (+ the table's full-text search) remains
  // the single source of truth — the bar reads from and writes to it. Only the
  ...
  // time/refresh controls stay. EventsTable only mounts on v4 Observations/
  // Traces surfaces, so the v4 gate is implicit.
  const searchBarMode =
    !hideControls &&
    !externalFilterState &&
    !peekContext &&
