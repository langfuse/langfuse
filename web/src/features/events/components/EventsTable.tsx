...
export default function ObservationsEventsTable({
  ...
  // tables (no longer a per-user Feature Preview opt-in). The sidebar's
  // FilterState (+ the table's full-text search) remains the single source of
  // truth — the bar reads from and writes to it. Only the legacy toolbar search
  // field is replaced (full-text search — bare text and content:/input:/output:
  // goes inline in the bar); the sidebar and time/refresh controls stay.
  const searchBarEnabled = true;
  const searchBarMode =
