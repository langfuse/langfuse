...
export function LargeJsonFieldFallback({
  ...
  const sizeSummary =
    rowCount != null
      ? t("rows", { count: compactNumberFormatter(rowCount, 1) })
      : t("characters", { count: compactNumberFormatter(charCount, 1) });
    ...
    <div className="io-message-content ph-no-capture">
      <div className="my-2 flex flex-col gap-2 rounded-sm border border-dashed p-3">
        <div className="text-muted-foreground flex flex-wrap items-center gap-2 text-xs">
          ...
          <span>{t("tooLarge", { size: sizeSummary })}</span>
        </div>
        <p className="text-muted-foreground text-xs">
          {t("largeJsonDescription")}
        </p>
