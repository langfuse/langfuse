...
export function TimelineV2({
  ...
  // fitted is the only view that means the same thing in every space.
  //
  // Compressed duration is part of that identity, not just the flags: collapsed
  // gaps are priced in PHYSICAL px (`min(28, width / (gaps + 1))`), so resizing
  // the pane while zoomed in with compression on rescales the whole space around
  // a window that still addresses the old one. Collapsing a node is deliberately
  // not in the key — it changes which rows exist, not what a ms means.
  const spaceKey = `${compress}:${compression.compressedDurationMs}`;
  const spaceRef = useRef({ roots, spaceKey });
  if (
    spaceRef.current.roots !== roots ||
    spaceRef.current.spaceKey !== spaceKey
  ) {
    spaceRef.current = { roots, spaceKey };
