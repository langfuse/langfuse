...
columns (${SCORE_COLUMNS.observation.numeric} / ${SCORE_COLUMNS.observation.categorical} / ${SCORE_COLUMNS.observation.boolean}): they match a score whether it was
recorded at observation OR trace level. Use the trace-only columns
(${SCORE_COLUMNS.trace.numeric} / ${SCORE_COLUMNS.trace.categorical} / ${SCORE_COLUMNS.trace.boolean}) ONLY when the request explicitly asks to scope to
trace-level scores.
...
- Boolean score: {"type": "booleanObject", "column": "${SCORE_COLUMNS.observation.boolean}" (any level) or "${SCORE_COLUMNS.trace.boolean}" (trace-only), "key": "<score name>", "operator": "=" | "<>", "value": true | false}
