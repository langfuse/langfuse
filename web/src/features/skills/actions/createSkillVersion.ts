...
type CreateVersion = (input: {
  projectId: string;
  files: Array<{ path: string; blobId: string; executable: boolean }>;
  labels: string[];
  tags: string[];
...
export async function createSkillVersionFromDraft(params: {
  ...
  const encodedFiles = await Promise.all(
    Object.values(draft.files).map(async (file) => {
  ...
  const prepared = await params.prepareUploads({
  ...
  return params.createVersion({
    projectId: params.projectId,
    files: encodedFiles.map(({ file, sha256Hash }) => ({
      path: file.path,
      blobId: uploadByHash.get(sha256Hash)!.blobId,
