...
export function DeleteFolder({ folderPath }: { folderPath: string }) {
  ...
  const skills = api.skills.all.useQuery(
    {
      ...
      page: 0,
      limit: 100, // Fetch up to 100 skills to show in the list
      filter: [],
  ...
  const mutDeleteFolder = api.skills.delete.useMutation({
    onSuccess: () => {
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <DialogBody>
          <p className="text-muted-foreground text-sm">
            ...
            and <b>all skills inside it recursively</b>. This cannot be undone.
            If a skill is still used in your application, your application will
            break.
          </p>

          <div className="bg-muted/50 rounded-md border p-4">
            <h4 className="mb-2 text-sm font-medium">Skills to delete:</h4>
            {skills.isLoading ? (
              ...
              <div className="py-2 text-xs text-red-500">
                Failed to load skills: {skills.error.message}
              </div>
            ) : (
              <ul className="max-h-32 space-y-1 overflow-y-auto text-xs">
                ...
                {(skills.data?.totalCount ?? 0) > 100 && (
                  <li className="text-muted-foreground pt-1 italic">
                    And {(skills.data?.totalCount ?? 0) - 100} more skills...
                  </li>
                )}
