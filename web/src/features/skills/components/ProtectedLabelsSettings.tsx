...
export default function ProtectedLabelsSettings({
  ...
  // Skills reuse the prompt protected-labels entitlement until a dedicated one
  // exists (mirrors the skill router).
  ...
  const { data: protectedLabels = [] } = api.skills.getProtectedLabels.useQuery(
    { projectId },
    {
  ...
  const { data: allLabels = [] } = api.skills.allLabels.useQuery(
    { projectId },
    {
      enabled: Boolean(projectId),
    },
  ...
  const availableLabels = allLabels.filter(
    (label) => !protectedLabels.includes(label) && label !== LATEST_SKILL_LABEL,
  );

  const addProtectedLabel = api.skills.addProtectedLabel.useMutation({
    onSuccess: () => {
      utils.skills.getProtectedLabels.invalidate();
      form.reset();
    },
  });

  const removeProtectedLabel = api.skills.removeProtectedLabel.useMutation({
    onSuccess: () => {
      utils.skills.getProtectedLabels.invalidate();
    },
  });
  ...
  return (
    <div>
      <Header title="Protected Skill Labels" />
