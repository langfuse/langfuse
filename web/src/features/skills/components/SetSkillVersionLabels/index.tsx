...
import { isReservedSkillLabel } from "@/src/features/skills/utils";
import { TruncatedLabels } from "@/src/components/TruncatedLabels";
import { cn } from "@/src/utils/tailwind";

export function SetSkillVersionLabels({
  skillLabels,
  skill,
  isOpen,
  setIsOpen,
  ...
  const labelsChanged =
    ...
    JSON.stringify([...skill.labels].sort());
  const customLabelScrollRef = useRef<HTMLDivElement | null>(null);
  const previousIsOpenRef = useRef(false);
  const previousSkillIdRef = useRef(skill.id);

  const usedLabelsInProject = api.skills.allLabels.useQuery(
    {
  ...
  useEffect(() => {
    ...
    const skillChangedWhileOpen =
      isOpen && previousSkillIdRef.current !== skill.id;

    if (justOpened || skillChangedWhileOpen) {
      setSelectedLabels(skill.labels);
      setCreatedLabels([]);
      setSearchValue("");
    }

    previousIsOpenRef.current = isOpen;
    previousSkillIdRef.current = skill.id;
  }, [isOpen, skill.id, skill.labels]);

  const isPromotingToProduction =
    !skill.labels.includes(PRODUCTION_LABEL) &&
  ...
  const mutateSkillVersionLabels = api.skills.setLabels.useMutation({
    onSuccess: () => {
  ...
  const handleSubmitLabels = async () => {
    try {
      ...
      await mutateSkillVersionLabels.mutateAsync({
        projectId: projectId as string,
        skillId: skill.id,
        labels: selectedLabels,
      });

      capture("skill_detail:apply_labels", { labels: selectedLabels });
      setIsOpen(false);
    } catch (err) {
      console.error(err);
    }
  ...
  const customLabels = labels.filter((l) => !isReservedSkillLabel(l));
  ...
  const isValidNewLabel =
    ...
    !isReservedSkillLabel(trimmedSearch) &&
    SkillLabelSchema.safeParse(trimmedSearch).success &&
  ...
  const handleCreateLabel = () => {
    ...
    capture("skill_detail:add_label_submit");
    setSearchValue("");
    setTimeout(
      () =>
  ...
  return (
    <Popover open={isOpen} onOpenChange={handleOnOpenChange} modal={false}>
      ...
      >
        ...
        >
          <h2 className="mb-3 font-semibold">Skill labels</h2>
          <h2 className="mb-3 text-xs">
            Use labels to fetch skills via SDKs. The <strong>production</strong>{" "}
            labeled skill will be served by default.
          </h2>
          ...
          <Button
            ...
            loading={mutateSkillVersionLabels.isPending}
