...
export const StructuredOutputSchemaSection = () => {
  ...
  const hasSchemaCudAccess = useHasProjectAccess({
    projectId,
  ...
  useEffect(() => {
    if (structuredOutputSchema && !structuredOutputSchema.existingLlmSchema) {
      const matchingSavedSchema = savedSchemas.find(
        (savedSchema: LlmSchema) => savedSchema.name === structuredOutputSchema.name,
      );
          ...
          <PopoverContent className="w-80 p-0" align="end">
            <Command>
              ...
              <CommandList>
                ...
                <CommandGroup>
                  {savedSchemas.map((schema: LlmSchema) => (
                    <CommandItem
                      key={schema.id}
