...
interface ColumnPlaygroundContextType extends MessagesContext, ModelParamsContext {
  ...
  setStructuredOutputSchema: (schema: PlaygroundColumnState['structuredOutputSchema']) => void;
  output: string;
  outputJson: string;
...
export const PlaygroundColumnProvider: React.FC<PlaygroundColumnProviderProps> = ({
  ...
  const updateMessage: MessagesContext['updateMessage'] = useCallback((_, id, key, value) => {
  ...
  const setMessages = useCallback((messages: ChatMessageWithId[]) => {
    updateColumnMessages(columnId, messages);
  }, [columnId, updateColumnMessages]);

  // Model params context methods
  const updateModelParamValue: ModelParamsContext['updateModelParamValue'] = useCallback((key, value) => {
  ...
  const setModelParamEnabled: ModelParamsContext['setModelParamEnabled'] = useCallback((key: keyof UIModelParams, enabled: boolean) => {
  ...
  // Tools methods
  const setTools = useCallback((tools: PlaygroundColumnState['tools']) => {
    updateColumnTools(columnId, tools);
  }, [columnId, updateColumnTools]);

  // Structured output methods
  const setStructuredOutputSchema = useCallback((schema: PlaygroundColumnState['structuredOutputSchema']) => {
    updateColumnSchema(columnId, schema);
  }, [columnId, updateColumnSchema]);

  // Message placeholders methods
  const updateMessagePlaceholderValue = useCallback((name: string, value: any) => {
  ...
  const deleteMessagePlaceholder = useCallback((name: string) => {
    const updatedPlaceholders = column.messagePlaceholders.filter(p => p.name !== name);
    updateColumnMessagePlaceholders(columnId, updatedPlaceholders);
  }, [column.messagePlaceholders, columnId, updateColumnMessagePlaceholders]);
...
// Export a hook that works with both the original context and the column context
export const usePlaygroundContext = () => {
  // Try to get the column context first
  const columnContext = useContext(ColumnPlaygroundContext);
  if (columnContext) {
    return columnContext;
  }
  
  // Fallback to checking if we're in the original single-column context
  // This will be imported from the original context file
  try {
    const { usePlaygroundContext: useOriginalContext } = require("../context");
    return useOriginalContext();
  } catch {
    throw new Error(
      "usePlaygroundContext must be used within a PlaygroundProvider or PlaygroundColumnProvider",
    );
  }
};