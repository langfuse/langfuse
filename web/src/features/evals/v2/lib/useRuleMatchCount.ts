 ...
 */
export function ruleTimeRangeFilter(
...
/**
 * Rough count of observations matching the rule filter within the view's
 * time range. Queried standalone (not derived from the rule preview table)
 * so the count keeps working if that table goes away.
 */
export function useRuleMatchCount({
  projectId,
  filterState,
  timeRange,
  enabled = true,
  ...
  const filter = useMemo<FilterState>(
    () => [...filterState, ...ruleTimeRangeFilter(timeRange)],
