...
import { VariableMappingPanel } from "./VariableMappingPanel/VariableMappingPanel";
...
type MappingSelectorKind = "tree" | "drill";
  ...
  return (
    <>
      ...
      <SampleDataTreeSelector
        ...
        roots={MAPPABLE_COLUMNS.map((column) => ({
          ...column,
          value: sourceObject[column.id],
        }))}
        onSelect={onSelect}
      />
    </>
 ...
 * clickable beyond the explicit header actions.
 */
function VariableMappingRow({
  ...
  if (editing && selector === "drill") {
    return (
      <div
        ref={focusEditingSurface}
        tabIndex={-1}
        className="bg-card flex flex-col rounded-md border outline-hidden"
      >
        {body}
      </div>
    );
