...
/**
 * Read-only preview of observations matching the rule filter, embedded the
 ...
 */
export function EvaluationRulePreviewTable({
  projectId,
  filterState,
  timeRange,
  onSelectObservation,
  ...
  // externalDateRange still rules the facet options.
  const effectiveFilterState = useMemo(
    () => filterState.concat(ruleTimeRangeFilter(timeRange)),
  ...
  return (
    <>
      ...
      {/* Exactly five fully visible rows: 40px header + 5 × 29px rows (h-7
          plus the 1px separator) + 3px of borders — more rows load in on
          scroll. */}
      <div
        className={cn(
          "flex max-h-[188px] w-full flex-col overflow-hidden border",
          isEmpty && "hidden",
        )}
      >
        <Suspense fallback={<Skeleton className="h-[188px] w-full" />}>
          <EventsTable
            projectId={projectId}
            hideControls
