...
const STREAMING_SEED_MESSAGES: InAppAgentWindowMessage[] = [
  {
    id: "seed-user-1",
    role: "user",
    content: {
...
const STREAMING_INVESTIGATIONS = [
  {
...
function StreamingInAppAgentWindow(args: InAppAgentWindowProps) {
  ...
  useEffect(() => {
    const intervalId = window.setInterval(() => {
      ...
      const investigation =
        STREAMING_INVESTIGATIONS[
          stream.cycle % STREAMING_INVESTIGATIONS.length
        ];
      const toolCalls = [
        {
...
const LONG_UNBROKEN_WORD = `trace-${"0123456789abcdef".repeat(18)}`;
const LONG_UNBROKEN_TABLE_VALUE = `observation-${"abcdefghijklmnopqrstuvwxyz".repeat(10)}`;
  ...
  args: {
    ...
    messages: [
      ...
      },
      {
        ...
        content: {
          type: "text",
          text: `This normal message includes a very long unbroken word to check wrapping: ${LONG_UNBROKEN_WORD}`,
        },
      },
      {
      ...
      },
      {
        ...
        content: {
          ...
          text: [
            ...
            "| Field | Value | Notes |",
            "| --- | --- | --- |",
            `| Trace ID | ${LONG_UNBROKEN_WORD} | Generated by a customer system without delimiters |`,
            `| Observation ID | ${LONG_UNBROKEN_TABLE_VALUE} | Long table cell value without spaces |`,
            `| Metadata key | custom-${LONG_UNBROKEN_TABLE_VALUE} | Another long unbroken token in a different column |`,
          ].join("\n"),
        },
      },
    ],
...
export const Streaming = meta.story({
  args: {
    isAssistantTurnInProgress: true,
    selectedConversationId: "conversation-1",
    messages: STREAMING_SEED_MESSAGES,
  },
