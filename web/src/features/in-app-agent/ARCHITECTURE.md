...
| **Replay messages**            | `getConversationMessagesForReplay` (`packages/shared/.../persistence.ts`)             | model context for a resuming run                                               | unsigned reasoning, redirect results, `runId`s and unpaired tool calls stripped; signed reasoning kept for Bedrock    | rendered, or used as a client hydration snapshot |
