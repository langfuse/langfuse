...
export function DuplicateFolder({ folderPath }: { folderPath: string }) {
  ...
  const form = useForm({
    resolver: zodResolver(
      formSchema.extend({
        targetPath: z.string().min(1, t("targetRequired")),
      }),
    ),
    defaultValues: {
          ...
          >
            <DialogBody>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-col gap-2">
                    <FormLabel>{t("targetPath")}</FormLabel>
                    <FormControl>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t("versionSettings")}</FormLabel>
                    <FormControl>
                      <RadioGroup
                        {...field}
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-row items-start space-y-0 space-x-3 rounded-md border p-3">
                    ...
                    <div className="space-y-1 leading-none">
                      <FormLabel>{t("rewriteReferences")}</FormLabel>
                      <FormDescription>
                        {t("rewriteBefore")}{" "}
