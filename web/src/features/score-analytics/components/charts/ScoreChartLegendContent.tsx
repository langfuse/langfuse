...
const LegendItem = ({
  ...
  return (
    <button
      ...
      aria-label={t(visible ? "hideLegendItem" : "showLegendItem", { label })}
      className={cn(
            ...
            <Popover open={showPopover} onOpenChange={setShowPopover}>
              <PopoverTrigger asChild>
                <Button
                  ...
                  aria-label={t("showAllCategories", { count: payload.length })}
                >
                  <span>{t("showAll", { count: payload.length })}</span>
                  {hiddenCount > 0 && (
              ...
              >
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <p className="text-sm font-bold">{t("allCategories")}</p>
                    <span className="text-muted-foreground text-xs">
                      {t("allCategoriesCount", { count: payload.length })}
                    </span>
                  </div>
                  <div className="space-y-3">
