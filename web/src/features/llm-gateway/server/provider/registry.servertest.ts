...
describe("LLM gateway provider registry", () => {
  ...
  it("reads model lists from an organization and connection scoped cache", async () => {
    ...
    const redis = {
      get: vi
        .fn()
        .mockResolvedValue(
          JSON.stringify([{ id: "gpt-5-mini", provider: "OPENAI" }]),
        ),
      setex: vi.fn(),
    } as unknown as Redis;
    const prisma = {
  ...
  it("collects every Anthropic model page with native metadata", async () => {
    const fetcher = vi
      .fn<typeof fetch>()
    ...
    await expect(
      ...
      models: [
        ...
        { id: "claude-b", provider: "ANTHROPIC", displayName: "Claude B" },
      ],
    });
    expect(String(fetcher.mock.calls[1]?.[0])).toContain("after_id=claude-a");
  });

  it("preserves OpenRouter metadata, provider order, and pagination links", async () => {
    const fetcher = vi
      .fn<typeof fetch>()
      .mockResolvedValueOnce(
        Response.json({
      ...
      models: [
        {
          ...
          nativeModel: { pricing: { prompt: "0.1", completion: "0.2" } },
        },
        { id: "openrouter/model-a", provider: "OPENROUTER" },
      ],
    });
  ...
  it("bypasses and refreshes the cache during an explicit sync", async () => {
    ...
    expect(redis.setex).toHaveBeenCalledWith(
      "llm-gateway:models:org-1:connection-1",
      300,
      JSON.stringify([{ id: "gpt-5", provider: "OPENAI" }]),
    );
  });
});
