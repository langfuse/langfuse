...
export class QueryBuilder {
  ...
  /** OceanBase/MySQL: quantile → AVG (approximation), histogram unsupported */
  private translateAggregation(metric: AppliedMetricType): string {
    switch (metric.aggregation) {
      ...
      case "p95":
      case "p99":
        // OceanBase has no quantile(); use AVG as approximation
        return `AVG(${metric.alias || metric.sql})`;
      case "histogram":
  ...
  private actualTableName(view: ViewDeclarationType): string {
    // When baseCte is "table alias" (e.g. "scores scores_numeric"), use the alias so WHERE/SELECT
    // reference the correct name (OceanBase/MySQL only allow the alias after FROM).
    const parts = view.baseCte.trim().split(/\s+/);
  ...
  private buildJoins(
    ...
    for (const relationTableName of relationTables) {
      ...
      // OceanBase: no FINAL modifier; optionally skip for observations if flag set (alias only)
      const alias =
        relation.name !== relationTableName ? ` AS ${relationTableName}` : "";
      const joinStatement = `LEFT JOIN ${relation.name}${alias} ${relation.joinConditionSql}`;
  ...
  /** OceanBase/MySQL: use DATE_FORMAT, DATE() instead of ClickHouse toStartOf* / toDate / toMonday */
  private getTimeDimensionSql(
    ...
    switch (granularity) {
      case "minute":
        return `DATE_FORMAT(${sql}, '%Y-%m-%d %H:%i:00')`;
      case "hour":
    ...
    if (appliedDimensions.length > 0) {
      dimensions += `${appliedDimensions
        .map((dimension) => {
          ...
          // Default: wrap in ANY_VALUE() (MySQL/OceanBase equivalent of ClickHouse any())
          return `ANY_VALUE(${dimension.sql}) as ${dimension.alias ?? dimension.sql}`;
        })
        .join(",\n")},`;
    }

    // Add time dimension if specified - reuse unified builder with ANY_VALUE() wrapper (MySQL/OceanBase)
    const timeDimensionSql = this.buildTimeDimensionSql(
      view,
      query,
      "ANY_VALUE",
    );
  ...
  private buildInnerMetricsPart(appliedMetrics: AppliedMetricType[]) {
    ...
    // Deduplicate by alias so we don't emit duplicate column names (e.g. four "latency" columns
    // for p50/p90/p95/p99). MySQL/OceanBase disallow duplicate column names in subquery result.
    ...
    return appliedMetrics
      .map((metric) => {
        ...
        const alias = metric.alias || metric.sql;
  ...
  private buildWithFillClause(
    ...
    parameters["fillFromDate"] = convertDateToDateTime(new Date(fromTimestamp));
    parameters["fillToDate"] = convertDateToDateTime(new Date(toTimestamp));

    // OceanBase/MySQL: no WITH FILL; return empty (gaps in time series are not filled)
    return "";
  }
