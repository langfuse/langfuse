...
import { QueryBuilder } from "@/src/features/query/serverOb/queryBuilder";
import { type QueryType, type ViewVersion } from "@/src/features/query/types";
import { getViewDeclaration } from "@/src/features/query/serverOb/dataModel";
...
export async function executeQuery(
  ...
  // Execute all queries in parallel (OceanBase: use adapter + convertFilterParamsToPositional)
    ...
    queriesToExecute.map(
      async (
        ...
        try {
          ...
          if (compiledQuery.includes("traces")) {
            return await measureAndReturn({
              ...
              input: {
                ...
                tags: { ...tags, operation_name: "executeQuery" },
              },
              fn: async (input) => {
                const { query: q, params: p } = convertFilterParamsToPositional(
                  input.query,
                  input.params,
                );
                return adapter.queryWithOptions<Record<string, unknown>>({
                  query: q,
                  params: p,
                  tags: input.tags,
                });
          ...
          return await adapter.queryWithOptions<Record<string, unknown>>({
            query: obQuery,
            params: obParams,
            tags,
          });
