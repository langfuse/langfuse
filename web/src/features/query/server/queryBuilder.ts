...
export class QueryBuilder {
  ...
  /**
   * Strips @@AGG@@ templates from SQL expressions, leaving only the inner expression.
   * Used for preflight queries where we need raw column values instead of aggregated values.
   */
  private stripAggTemplates(
    sql: string,
    aggs?: Record<string, string>,
  ): string {
    if (!aggs) return sql;
    // Create a map with empty strings for all placeholders
    const emptyAggs = Object.fromEntries(
      Object.keys(aggs).map((key) => [key, ""]),
    );
    return this.substituteAggTemplates(sql, emptyAggs);
  }
  ...
  /**
   * Builds WHERE conditions for preflight queries to check if any rows have
   * non-zero/non-empty measure values.
   *
   * @returns SQL condition string like "(measure1 <> 0 OR measure2 <> 0)" or null if skip preflight
   */
  private buildPreflightConditions(
    appliedMetrics: AppliedMetricType[],
    view: ViewDeclarationType,
  ): string | null {
    // Filter out count aggregations (count can never be truly empty)
    const nonCountMetrics = appliedMetrics.filter(
      (m) => m.aggregation !== "count",
    );

    if (nonCountMetrics.length === 0) {
      return null; // Skip preflight - all metrics are counts
    }

    const conditions = nonCountMetrics.map((metric) => {
      // Strip @@AGG@@ templates to get raw SQL expression
      const rawSql = this.stripAggTemplates(metric.sql, metric.aggs);

      // Get measure type from view definition to determine zero value
      const measureDef = view.measures[metric.measureName];
      const measureType = measureDef?.type || "number";

      // Build condition based on type
      if (measureType === "string") {
        return `(${rawSql}) <> ''`;
      }
      return `(${rawSql}) <> 0`;
    });
  ...
  /**
   * Builds a preflight query to check if any rows have non-zero/non-empty measure values.
   * This is used for high-cardinality dimension queries to verify the query won't produce
   * a large result set with only zero/empty values.
   *
   * @param query - The query configuration
   * @param projectId - The project ID
   * @param measuresToCheck - List of measure names to check (filters to only these measures)
   * @returns The preflight query and parameters, or null if preflight should be skipped
   */
  public buildPreflightQuery(
    query: QueryType,
    projectId: string,
    measuresToCheck: string[],
  ): { query: string; parameters: Record<string, unknown> } | null {
    // Run zod validation
    ...
    // Map metrics to get their SQL expressions
    const appliedMetrics = this.mapMetrics(query.metrics, view);

    // Filter to only the measures we need to check
    const metricsToCheck = appliedMetrics.filter((m) =>
      measuresToCheck.includes(m.measureName),
    );

    // Build preflight conditions for non-zero/non-empty measures
    const preflightConditions = this.buildPreflightConditions(
      metricsToCheck,
      view,
    );

    // If no preflight conditions (all counts), skip preflight
    if (!preflightConditions) {
      return null;
    }

    // Create filter list from user-provided filters
    ...
    // For preflight, we only need relation tables if the measures being checked use them
    const relationTables = new Set<string>();
    ...
    if (relationTables.size > 0) {
      const relationJoins = this.buildJoins(
        ...
        filterList,
        query,
        true, // Always skip FINAL for preflight
      );
      fromClause += ` ${relationJoins.join(" ")}`;
    }

    // Build WHERE clause with user filters + standard filters
    const whereClause = this.buildWhereClause(filterList, parameters);

    // Build the preflight query
    const sql = `
      SELECT 1
      ${fromClause}
      ${whereClause}
      AND ${preflightConditions}
      LIMIT 1`;
