...
import {
  ...
  CreateTraceAddToDatasetActionSchema,
} from "../validation";
...
export const addToDatasetRouter = createTRPCRouter({
  ...
  createForTraces: protectedProjectProcedure
    .input(CreateTraceAddToDatasetActionSchema)
    .mutation(async ({ input, ctx }) => {
      try {
        // Check permissions
        throwIfNoProjectAccess({
        ...
        // Check trace count doesn't exceed maximum
        const traceCount = await getTracesTableCount({
          projectId,
          filter: query.filter ?? [],
        ...
        if (traceCount > MAX_BATCH_ADD_TO_DATASET_ITEMS) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: `Too many traces selected. Maximum allowed is ${MAX_BATCH_ADD_TO_DATASET_ITEMS}, but ${traceCount} traces match your filters. Please refine your filters to reduce the count.`,
          });
        }
        const userId = ctx.session.user.id;

        logger.info("[TRPC] Creating trace-add-to-dataset action", {
          projectId,
        });
        ...
        const batchAction = await ctx.prisma.batchAction.create({
          data: {
            projectId,
            userId,
            actionType: ActionId.TraceAddToDataset,
        ...
        await BatchActionQueue.getInstance()?.add(
          QueueJobs.BatchActionProcessingJob,
          {
            ...
            payload: {
              batchActionId: batchAction.id,
              projectId,
              actionId: ActionId.TraceAddToDataset,
              tableName: BatchTableNames.Traces,
              cutoffCreatedAt: new Date(),
              query,
              config,
        ...
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: "Creating trace-add-to-dataset action failed.",
        });
      }
    }),
});
