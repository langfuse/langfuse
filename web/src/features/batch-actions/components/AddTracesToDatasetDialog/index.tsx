...
// Reuse step components from observations dialog
import { DatasetChoiceStep } from "../AddObservationsToDatasetDialog/DatasetChoiceStep";
import { DatasetSelectStep } from "../AddObservationsToDatasetDialog/DatasetSelectStep";
import { DatasetCreateStep } from "../AddObservationsToDatasetDialog/DatasetCreateStep";
import { MappingStep } from "../AddObservationsToDatasetDialog/MappingStep";
import { FinalPreviewStep } from "../AddObservationsToDatasetDialog/FinalPreviewStep";
import { StatusStep } from "../AddObservationsToDatasetDialog/StatusStep";

// Hook
import { useAddTracesToDatasetWizard } from "./useAddTracesToDatasetWizard";

type AddTracesToDatasetDialogProps = {
  projectId: string;
...
export function AddTracesToDatasetDialog(props: AddTracesToDatasetDialogProps) {
  ...
  const {
    ...
    canClose,
    isLoading,
  } = useAddTracesToDatasetWizard(props);
  ...
  return (
    <Dialog open onOpenChange={(open) => !open && canClose && onClose()}>
      <DialogContent className="flex max-h-[90vh] max-w-6xl flex-col">
        <DialogHeader>
          <DialogTitle>
            Add {displayCount} Trace(s) to dataset
