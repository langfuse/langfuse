...
- `constants/scoresSearchRegistry.ts` derives the grammar from the Scores
  sidebar's facets. Bare text searches within score names; exact selections,
...
- `components/table/use-cases/scores.tsx` (under `src/`) owns data fetching,
  URL/saved-view filters, and the table. It mounts the bar only for the full-page
  v4 table; scoped embeds and chart mode have different filter contracts.
