...
export const _handleGenerateScoresForPublicApi = async ({
  ...
  // OceanBase/MySQL: use ROW_NUMBER() instead of LIMIT 1 BY. Use s_inner prefix in inner filter.
  const innerScoreFilter = appliedScoresFilter.query
    ? appliedScoresFilter.query.replace(/\bs\./g, "s_inner.")
    : "";
  const innerSubquery = `
    SELECT trace_id, project_id FROM (
      SELECT s_inner.trace_id, s_inner.project_id,
        ROW_NUMBER() OVER (PARTITION BY s_inner.id, s_inner.project_id ORDER BY s_inner.timestamp desc) as rn
      FROM scores s_inner
      WHERE s_inner.project_id = {projectId: String}
        ${innerScoreFilter ? `AND ${innerScoreFilter}` : ""}
        ${scoreScope === "traces_only" ? "AND s_inner.session_id IS NULL AND s_inner.dataset_run_id IS NULL" : ""}
    ) x WHERE rn = 1
  `;
  const query = `
      SELECT
          ${needsTraceJoin ? "t.user_id as user_id, t.tags as tags, t.environment as trace_environment," : ""}
          s.id as id, s.project_id as project_id, s.timestamp as timestamp, s.environment as environment,
          s.name as name, s.value as value, s.string_value as string_value, s.long_string_value as long_string_value,
          s.author_user_id as author_user_id, s.created_at as created_at, s.updated_at as updated_at,
          s.source as source, s.comment as comment, s.metadata as metadata, s.data_type as data_type,
          s.config_id as config_id, s.queue_id as queue_id, s.trace_id as trace_id, s.observation_id as observation_id,
          s.session_id as session_id, s.dataset_run_id as dataset_run_id
      FROM (
        SELECT s.*, ROW_NUMBER() OVER (PARTITION BY s.id, s.project_id ORDER BY s.timestamp desc, s.event_ts desc) as rn
        FROM scores s
        ${needsTraceJoin ? "LEFT JOIN traces t ON s.trace_id = t.id AND s.project_id = t.project_id" : ""}
        WHERE s.project_id = {projectId: String}
          AND ( ${scoreScope === "traces_only" ? "" : "s.trace_id IS NULL OR "}
            (s.trace_id IS NOT NULL AND (s.trace_id, s.project_id) IN (${innerSubquery})) )
      ...
      ${needsTraceJoin ? "LEFT JOIN traces t ON s.trace_id = t.id AND s.project_id = t.project_id" : ""}
      WHERE s.rn = 1
      ORDER BY s.timestamp desc, s.event_ts desc
  ...
  const { query: obQuery, params: obParams } = convertFilterParamsToPositional(
    query.trim().replace(/__TRACE_TABLE__/g, "traces"),
    params,
  );
  ...
  return measureAndReturn({
    ...
    fn: async (inp) => {
      ...
      return records.map((record) => {
        // Normalize OB datetime to string for CH parser if needed
        ...
        const normalized =
          ts instanceof Date
            ...
            : record;
        const domainScore = convertClickhouseScoreToDomain(normalized);
        const apiScore = convertScoreToPublicApi(domainScore);
        return {
          ...apiScore,
          trace:
            inp.includeTrace && record.trace_id !== null
              ? {
...
export const _handleGetScoresCountForPublicApi = async ({
  ...
  // OceanBase/MySQL: use ROW_NUMBER() instead of LIMIT 1 BY
  const innerScoreFilterCount = appliedScoresFilter.query
    ? appliedScoresFilter.query.replace(/\bs\./g, "s_inner.")
    : "";
  const innerSubquery = `
    SELECT trace_id, project_id FROM (
      ...
      FROM scores s_inner
      WHERE s_inner.project_id = {projectId: String}
        ${innerScoreFilterCount ? `AND ${innerScoreFilterCount}` : ""}
        ${scoreScope === "traces_only" ? "AND s_inner.session_id IS NULL" : ""}
    ) x WHERE rn = 1
  `;
  const query = `
