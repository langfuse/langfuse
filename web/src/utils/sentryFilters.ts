...
const NOISE_MESSAGE_PREFIXES: readonly string[] = [
  ...
  "[PostHog.js]",
  // Cloudflare Kitesurf console-errors its own localhost CORS probe with this
  // unbracketed prefix (LANGFUSE-610). Distinct from the `[kitesurf]` listener
  // wrap (LANGFUSE-60W) — do not add that bracketed prefix here; a real
  // listener throw the injector wrapped must still reach Sentry. Covered by
  // the dedicated 60W/60X predicates in sibling PRs.
  "kitesurf:",
