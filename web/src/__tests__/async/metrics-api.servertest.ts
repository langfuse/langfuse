...
// Test suite factory to run tests against both V1 and V2 endpoints
const runTestSuite = (version: "v1" | "v2") => {
  const endpoint =
    version === "v2" ? "/api/public/v2/metrics" : "/api/public/metrics";
  const responseSchema =
    version === "v2" ? GetMetricsV2Response : GetMetricsV1Response;
  const suiteName = version === "v2" ? "V2" : "V1";

  describe(`/api/public/${version === "v2" ? "v2/" : ""}metrics API Endpoint (${suiteName})`, () => {
    ...
    beforeAll(async () => {
      ...
      // Time multiplier: events table uses microseconds, observations use milliseconds
      const timeMultiplier = version === "v2" ? 1000 : 1;
        ...
        if (version === "v2") {
          trace1Observations.push(
            createEvent({
              ...
              name: `observation-${i}`,
              type: "SPAN",
              start_time: baseTime * timeMultiplier,
              metadata_names: ["test"],
      ...
      for (let i = 0; i < 2; i++) {
        const obsId = randomUUID();
        const baseTime = yesterday.getTime();
      ...
      // Insert observations into appropriate table
      if (version === "v2") {
        await createEventsCh([...trace1Observations, ...trace2Observations]);
      } else {
    ...
    it.each([
      ...
      [
        ...
        false, // usesTracesView - this test can run on V2!
      ],
    ])(
      ...
      async (
        ...
        usesTracesView: boolean,
      ) => {
        // Skip this test for V2 if it uses traces view
        if (version === "v2" && usesTracesView) {
          return;
        }
        ...
        const response = await makeZodVerifiedAPICall(
          responseSchema,
          "GET",
          `${endpoint}?query=${encodeURIComponent(JSON.stringify(fullQuery))}`,
        );
    ...
    it("should return 401 with invalid authentication", async () => {
      // Skip for V2 - this test uses traces view
    ...
    it("should return 400 for invalid query parameters", async () => {
      ...
      const { status } = await makeZodVerifiedAPICallSilent(
        responseSchema,
        "GET",
        `${endpoint}?query=invalid-json-string`,
        undefined,
      );
    ...
    it("should return 400 for incomplete query object", async () => {
      ...
      const { status } = await makeZodVerifiedAPICallSilent(
        responseSchema,
        "GET",
        `${endpoint}?query=${encodeURIComponent(JSON.stringify(incompleteQuery))}`,
        undefined,
      );
    ...
    it("should return 400 for invalid metric measure", async () => {
      ...
      const { status, body } = await makeAPICall(
        "GET",
        `${endpoint}?query=${encodeURIComponent(JSON.stringify(invalidMetricNameQuery))}`,
        undefined,
      );
    ...
    it("should return 400 for invalid metric aggregation", async () => {
      ...
      const { status, body } = await makeAPICall(
        "GET",
        `${endpoint}?query=${encodeURIComponent(JSON.stringify(invalidAggregationQuery))}`,
        undefined,
      );
    ...
    it("should return 400 for invalid dimension", async () => {
      ...
      const { status, body } = await makeAPICall(
        "GET",
        `${endpoint}?query=${encodeURIComponent(JSON.stringify(invalidDimensionQuery))}`,
        undefined,
      );
    ...
    it("should return 400 for invalid filter column", async () => {
      ...
      const { status, body } = await makeAPICall(
        "GET",
        `${endpoint}?query=${encodeURIComponent(JSON.stringify(invalidFilterQuery))}`,
        undefined,
      );
    ...
    it("should handle histogram aggregation with custom bin count", async () => {
      // This test uses observations view - runs on both V1 and V2!
      ...
      const timeMultiplier = version === "v2" ? 1000 : 1;
      const histogramObservations: Array<
        ReturnType<typeof createObservation> | ReturnType<typeof createEvent>
      > = [];
      const costValues = [
        ...
        if (version === "v2") {
          histogramObservations.push(
            createEvent({
              ...
              type: "GENERATION",
              start_time: now.getTime() * timeMultiplier,
              total_cost: cost,
              metadata_names: ["test"],
              metadata_raw_values: [testMetadataValue],
            }),
      ...
      if (version === "v2") {
        ...
        await createObservationsCh(
          histogramObservations as ReturnType<typeof createObservation>[],
        );
      }
      ...
      const response = await makeZodVerifiedAPICall(
        responseSchema,
        "GET",
        `${endpoint}?query=${encodeURIComponent(JSON.stringify(histogramQuery))}`,
      );
    ...
    describe("LFE-6148: Comprehensive filter validation", () => {
      it("should return 400 error for invalid array field filters", async () => {
        ...
        const response = await makeAPICall(
          "GET",
          `${endpoint}?query=${encodeURIComponent(JSON.stringify(invalidStringTypeQuery))}`,
        );
      ...
      it("should return 400 error for invalid metadata filters", async () => {
        ...
        const response = await makeAPICall(
          "GET",
          `${endpoint}?query=${encodeURIComponent(JSON.stringify(invalidMetadataTypeQuery))}`,
        );
      ...
      it("should work correctly with proper array field filter configuration", async () => {
        ...
        const response = await makeZodVerifiedAPICall(
          responseSchema,
          "GET",
          `${endpoint}?query=${encodeURIComponent(JSON.stringify(validQuery))}`,
        );
...
// Run test suite for both V1 and V2 endpoints
runTestSuite("v1");
runTestSuite("v2");
