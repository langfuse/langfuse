...
describe("/api/public/score-configs API Endpoint", () => {
  ...
  it("should GET all score configs", async () => {
    ...
    expect(fetchedConfigs.body.data[0]?.id).toBe(CONFIG_ID_ONE);
    expect(fetchedConfigs.body.data[0]?.categories).toEqual([
      { label: "False", value: 0 },
      { label: "True", value: 1 },
    ]);
    expect(fetchedConfigs.body.data[0]?.maxValue).toBe(undefined);
    expect(fetchedConfigs.body.data[0]?.minValue).toBe(undefined);
  });
});
