...
describe("/api/public/v2/observations API Endpoint", () => {
  ...
  maybe("subquery rewrite", () => {
    ...
    it("handles duplicate versions of a span (same identity, newer event_ts) identically on both paths", async () => {
      // Updates to a span produce a second events_* row that differs only by
      // event_ts (ReplacingMergeTree version column). Neither path dedupes
      // (no FINAL / LIMIT 1 BY), so until a background merge collapses the
      // versions both rows are visible; this test pins that both paths see
      // the same multiset either way.
      const traceId = randomUUID();
      const obsId = randomUUID();
      ...
      const baseEvent = {
        ...
        trace_id: traceId,
        project_id: projectId,
        name: "dup-version-obs",
        type: "GENERATION" as const,
        level: "DEFAULT" as const,
        start_time: startMicros,
        input: `input-${"x".repeat(300)}`,
        output: `output-${"y".repeat(300)}`,
      };
      const eventTs = Date.now();
      // Two inserts -> two parts, so both versions exist until merged.
      await createEventsCh([createEvent({ ...baseEvent, event_ts: eventTs })]);
      await createEventsCh([
        createEvent({
          ...baseEvent,
          event_ts: eventTs + 1000,
          output: `output-updated-${"z".repeat(300)}`,
        }),
      ]);

      const url = `/api/public/v2/observations?traceId=${traceId}&fields=core,basic,io`;
      ...
      expect(ctePath.status).toBe(200);
      expect(subqueryPath.status).toBe(200);
      // All returned rows belong to the seeded span.
      expect(
        subqueryPath.body.data.every((o: { id: string }) => o.id === obsId),
      ).toBe(true);
      expect(subqueryPath.body.data.length).toBeGreaterThanOrEqual(1);
      // Both paths must surface the same rows (count and content), whether or
      // not ClickHouse has merged the duplicate versions yet.
      const byOutput = (rows: Array<{ output: unknown }>) =>
        [...rows].sort((a, b) =>
          JSON.stringify(a.output).localeCompare(JSON.stringify(b.output)),
        );
      expect(subqueryPath.body.data.length).toBe(ctePath.body.data.length);
      expect(byOutput(subqueryPath.body.data)).toEqual(
        byOutput(ctePath.body.data),
      );
    });
  });
});
