  ...
  describe("PATCH /api/public/score-configs/:configId", () => {
    ...
    it("should filter score configs by fromTimestamp and toTimestamp", async () => {
      ...
      response.body.data.forEach((config) => {
        expect(new Date(config.createdAt).getTime()).toBeGreaterThanOrEqual(
          new Date(from).getTime(),
        );
        expect(new Date(config.createdAt).getTime()).toBeLessThan(
          new Date(to).getTime(),
        );
      });
    });
    ...
    it("should reject non-ISO timestamp filters", async () => {
      const response = await makeAPICall(
        "GET",
        "/api/public/score-configs?fromTimestamp=2024%2F01%2F01",
        undefined,
        auth,
      );
