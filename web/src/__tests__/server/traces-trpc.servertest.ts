...
describe("traces trpc", () => {
  ...
  describe("traces flags", () => {
    ...
    // Reproduces the events_only-mode regression: a trace ingested after the
    // legacy->events cutover lives ONLY in the events table. The mutation read
    // must route to the events table (matching every other web read path,
    // gated on LANGFUSE_MIGRATION_V4_ALLOW_PREVIEW_OPT_IN); otherwise the legacy
    // `traces` read returns undefined and bookmark silently no-ops.
    (useEventsTable ? it : it.skip)(
      "should bookmark a trace that only exists in the events table",
      async () => {
    ...
    // Same root cause as the bookmark case above, but publish throws NOT_FOUND
    // (rewrapped as INTERNAL_SERVER_ERROR) instead of silently no-opping.
    (useEventsTable ? it : it.skip)(
      "should make a trace public that only exists in the events table",
      async () => {
        const traceId = randomUUID();
