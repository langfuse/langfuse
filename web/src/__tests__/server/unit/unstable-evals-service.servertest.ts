...
vi.mock(
  "../../../features/evals/v2/server/evaluators/evaluatorValidation",
  () => ({
    assertEvaluatorConfigurationValid: mockAssertEvaluatorConfigurationValid,
  }),
);
...
describe("unstable public eval services", () => {
  ...
  it("rejects unrunnable evaluator submissions before writing", async () => {
    mockAssertEvaluatorConfigurationValid.mockRejectedValueOnce(
      new EvaluatorConfigurationError("No valid LLM model found for evaluator"),
    );
