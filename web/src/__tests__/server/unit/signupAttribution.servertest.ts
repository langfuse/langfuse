...
describe("getAdClickIdsFromRequest", () => {
  ...
  it("reads the dedicated lf_* cookies for all platforms", () => {
    expect(
  ...
  it("omits undefined ids so the result can be spread into event properties", () => {
    expect(
      getAdClickIdsFromRequest({
        cookies: { lf_rdt_cid: "reddit-only" },
      }),
  ...
  it("reads the Reddit Pixel's _rdt_cid cookie", () => {
    expect(
      getAdClickIdsFromRequest({
        cookies: { _rdt_cid: "t2_reddit.click_id" },
      }),
