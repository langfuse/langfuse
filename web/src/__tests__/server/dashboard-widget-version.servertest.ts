...
describe("dashboard widget minVersion", () => {
  ...
  // ── tRPC uiHidden measure validation ─────────────────────────────────

  describe("tRPC uiHidden measure validation", () => {
    it.each([
      ["observations", "startTime", "min", 2],
      ["scores-numeric", "timestamp", "min", 2],
      ["scores-categorical", "timestamp", "min", 2],
    ] as const)(
      "create with measure '%s.%s' is rejected",
      async (view, measure, agg, minVersion) => {
        const caller = makeCaller();
        await expect(
          caller.dashboardWidgets.create({
            projectId,
            name: "uiHidden measure test widget",
            description: `${measure} measure on ${view}`,
            view,
            dimensions: [],
            metrics: [{ measure, agg }],
            filters: [],
        ...
        ).rejects.toThrow(/Measures not available for widgets/);
      },
    );

    it("should validate uiHidden measures on update mutation", async () => {
      const caller = makeCaller();
      const created = await caller.dashboardWidgets.create({
        projectId,
        name: "Widget to update",
        description: "will try hidden measure update",
        view: "observations",
        dimensions: [],
        metrics: [{ measure: "count", agg: "count" }],
        filters: [],
      ...
      await expect(
        caller.dashboardWidgets.update({
          ...
          widgetId: created.widget.id,
          name: "Widget to update",
          description: "hidden measure update",
          view: "observations",
          dimensions: [],
          metrics: [{ measure: "startTime", agg: "min" }],
          filters: [],
          chartType: "NUMBER",
          chartConfig: { type: "NUMBER" },
          minVersion: 2,
