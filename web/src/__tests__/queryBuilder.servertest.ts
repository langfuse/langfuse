...
describe("queryBuilder", () => {
  ...
  describe("query result correctness", () => {
    ...
    describe("traces view", () => {
      ...
      it("LFE-6148: should not have position() function error with tags", async () => {
        // This test reproduces the specific error:
        // "Illegal type Array(String) of argument of function position"
        // Setup
        const projectId = randomUUID();
        const tracesData = [
          {
            name: "trace-with-target-tag",
            tags: ["target-tag", "other-tag"],
          },
          {
        ...
        // Define query that reproduces the error - filtering by specific tag with time dimension
        // This combination causes buildWithFillClause to generate position() calls on Array(String)
        const query: QueryType = {
          view: "traces",
          dimensions: [{ field: "name" }],
          metrics: [{ measure: "count", aggregation: "count" }],
        ...
        const foundTrace = result.data.find(
          (row: any) => row.name === "trace-with-target-tag",
        );
        expect(foundTrace).toBeDefined();
        expect(foundTrace.count_count).toBe("1");
      });
