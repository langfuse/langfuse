...
- Make impossible states impossible. fullWidth only exists for primary and
  secondary, so the union type forbids it elsewhere, not a runtime check.
...
- Check both themes for every state. Dark mode surfaced an unreadable hover
  ...
  never showed.
- Hover colors are tokens like every other color. The Button started with
  brightness filters because they felt cheap to maintain; they dodged the
  token system, needed per-theme direction tweaks and produced invisible
  hovers. Every hover fill now routes through a token (bg-elevation-4,
  bg-inverse-hover, bg-status-error-fill).
