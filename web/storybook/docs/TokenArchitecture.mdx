...
- The mirror is a starting point, not the final word. Perception differs
  between dark and light backgrounds and across hue groups, so every pair
  gets checked by eye in both themes (status-error is red-800/red-400, not
  the mirror).
