// Multi-run engine comparison: alternates ch/rust runs (plus one discarded
// warmup pair) so thermal and background drift spread across both engines,
// then reports median [min..max] per metric.
//
// Usage: node bench.mjs [runs=6] [concurrency=4]
...
const results = { ch: [], rust: [] };
for (let i = 0; i <= RUNS; i++) {
  for (const engine of ["ch", "rust"]) {
...
const col = (f, digits) =>
  ["ch", "rust"]
    .map((e) => `${e}: ${fmt(results[e].map(f), digits)}`)
    .join("   ");
...
console.log(
  `worker          rust only: cpu_s ${fmt(results.rust.map((r) => r.worker_cpu_s))}, ` +
    `rss_mib ${fmt(
      results.rust.map((r) => r.worker_rss_mib),
      0,
    )}`,
);
console.log(
  `per-batch insert_ms (all windows pooled)   ch: ${fmt(
    results.ch.flatMap((r) => r.insert_ms),
    0,
  )}   rust: ${fmt(
    results.rust.flatMap((r) => r.insert_ms),
    0,
  )}`,
);
