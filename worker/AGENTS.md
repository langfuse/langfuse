...
- Trace-read micro-batching: `src/features/traces/traceBatching.ts` tracks
  ...
  hydration window by compatible observed start-time ranges; `project` remains
  the default and immediate rollback path. The locality selector is
  deterministic, seeds batches by readiness age, and flushes every hydrated
  candidate in the same dispatch run.
