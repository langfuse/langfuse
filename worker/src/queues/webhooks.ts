...
async function executeWebhookAction({
  ...
  if (input.payload.type === "prompt-version") {
    ...
    // metric-alert and future payload types: send as-is
    webhookPayload = JSON.stringify({
      id: input.executionId,
      timestamp: new Date(),
      ...input.payload,
...
async function executeGitHubDispatchAction({
  ...
  if (input.payload.type === "prompt-version") {
    ...
    // metric-alert and future payload types: send as-is in client_payload
    githubPayload = JSON.stringify({
      event_type: eventType,
      client_payload: {
        id: input.executionId,
...
/**
 * Execute PagerDuty action — sends alert to PagerDuty Events API v2
 */
...
/**
 * Execute Microsoft Teams action — sends Adaptive Card to incoming webhook URL
 */
...
/**
 * Execute Jira action — creates an issue via Jira REST API v3
 */
