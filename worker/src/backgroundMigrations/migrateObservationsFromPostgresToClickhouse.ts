  ...
  async run(args: Record<string, unknown>): Promise<void> {
    ...
    while (
      ...
      >(Prisma.sql`
        ...
        WHERE o.created_at <= '2023-11-10T01:26:24.732Z'
        AND o.id >= ${idIterator}
        ORDER BY id
        LIMIT ${batchSize};
      `);
      ...
      idIterator = observations[observations.length - 1].id;
      logger.info(
        `Processed batch in ${Date.now() - fetchStart}ms. ID iterator: ${idIterator}`,
      );
    }
