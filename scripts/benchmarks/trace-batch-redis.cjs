...
const source = fs.readFileSync(
  `${repo}/worker/src/features/traceBatching/traceBatching.ts`,
  "utf8",
);
const extract = (name) => {
...
(async () => {
  try {
    ...
    emit({
      ...
      trackScriptSha256: createHash("sha256").update(track).digest("hex"),
      snapshotScriptSha256: createHash("sha256").update(snapshot).digest("hex"),
      projectIdLength: 25,
      traceIdLength: 32,
      revisionLength: 36,
